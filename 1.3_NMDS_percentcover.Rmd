...
### library
```{r install libraries, message=FALSE, echo=FALSE}
...
```{r transpose percent cover data by lifeform, include=TRUE}

abcover_bystatuslifecycle <- abcover_full_v_omitground %>% 
  filter(lifeform == "forb" | lifeform == "grass") %>% 
   filter(lifecycle == "annual" | lifecycle == "perennial") %>% 
   mutate(status_lifecycle = paste(status, lifecycle, sep = "_")) %>% 
  
    group_by(site, site_rep, status_lifecycle) %>% 
  dplyr::summarize(
    count_per_transect = sum(n_distinct(site_rep_dist)), 
    percentcover = (count_per_transect/41)
    ) %>% 
  ...
  mutate(site_rep = case_when(site_rep == "1.1" ~ "1.01", # old name ~ new name
   ...
   TRUE ~ site_rep)) %>% 
    left_join(groups, by = "site_rep") %>%  
  dplyr::select(!c(group_15, group_3, group_10, group_6, group_4, group_2)) %>% 
   mutate(cluster = recode(cluster,
                          ...
                          "2" = "ssF",
                          "3" = "ssG"),
         cluster = factor(cluster, levels = c("EVG", "ssFG", "ssF", "ssG", "NNG"))) %>% 
  mutate(status_lifecycle = factor(status_lifecycle, levels = c("native_perennial", "native_annual", "nonnative_annual")))


status_lifecycle_means <- abcover_bystatuslifecycle %>% 
  group_by(cluster, status_lifecycle) %>% 
...
hist(abcover_bystatuslifecycle$percentcover)
shapiro.test(abcover_bystatuslifecycle$percentcover) # <-- not normal
kruskal.test(percentcover ~ status_lifecycle, data = abcover_bystatuslifecycle) 
# 	Kruskal-Wallis rank sum test
# 
# data:  percentcover by status_lifecycle
# Kruskal-Wallis chi-squared = 43.526, df = 3, p-value = 1.903e-09
pairwise.wilcox.test(abcover_bystatuslifecycle$percentcover, abcover_bystatuslifecycle$status_lifecycle, p.adjust.method = "BH")

status_lifecycle_plot <- ggplot(abcover_bystatuslifecycle, 
       aes(x = status_lifecycle, y = percentcover * 100, fill = status_lifecycle)) +
  geom_boxplot() +
   facet_wrap(~ cluster, nrow = 1) +
  labs(x = "Status and Lifecycle", y = "Percent Cover", fill = "Status Lifecycle") +
  ...
  scale_fill_manual(values = c("native_perennial" = "black", 
                               "native_annual" = "grey", 
                               "nonnative_annual" = "white"))

status_lifecycle_plot
```


```{r}

print("wonderful! You're at the end!")
...
```{r byplot - ggplot vectors and measurments, include = FALSE, eval = FALSE}
...
```{r ab percent cover unby species, inclue = TRUE, eval = FALSE}
...
```{r transpose percent cover data by species, include=TRUE, eval = FALSE}
...
```{r write nmds to csv, eval = FALSE}
...
```{r FROM KIT - nmds by species percent cover using code from KIT, eval = FALSE}
# USE THIS ONE!
...
```{r FROM KIT - nmds by site percent cover, eval = FALSE}
