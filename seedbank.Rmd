...
seedbank$stand_core_depth_treat_familydiversity <- paste(seedbank$stand_core_depth_treat, seedbank$family, sep="_") 
...
#  [1] "Artemisia californica"      "Ceanothus oliganthus"       "Diplacus aurantiacus"      
#  [4] "Eriodictyon crassifolium"   "Malacothamnus fasciculatus" "Nicotiana glauca"          
#  [7] "Ribes spp."                 "Salvia leucophylla"         "Unknown A - shrub"         
# [10] "Unknown D - brown dicot"   
sort(unique(shrub_seedbank$family)) 
length(unique(shrub_seedbank$family)) # 7 families
 
glimpse(shrub_seedbank)


shrub_seedbank %>%  # <---  run this code or... 
  filter(species_diversity == "Malacothamnus fasciculatus") %>% 
  group_by(site) %>% 
  summarize(total_count = sum(count))
# Ceanothus oliganthus = INT2_6
# Ribes spp. = INT
# Unknown A - shrub = INT1, INT3
# 
# Salvia leucophylla = INT, ENH
# Diplacus aurantiacus = ENH1_1
# Eriodictyon crassifolium = ENH1_1, ENH1_4
# Unknown D - brown dicot = ENH2_6
# 
# Nicotiana glauca = DEG, INT
# Artemisia californica = DEG, ENH, INT
# Malacothamnus fasciculatus = DEG, ENH, INT

shrub_seedbank %>% # <--- ... you can also run this code
  filter(species_diversity == "Ceanothus oliganthus") %>% 
  distinct(stand_core) %>%
  pull(stand_core)
...
boxplot(seeds_per_g_log ~ stand_core, data=germ_tray, main="Seeds/g (log) by hillslope and rep",
   xlab="Hillslope_rep", ylab="Seeds per gram")

boxplot(seeds_per_g_log ~ site, data=germ_tray, main="Seeds/g (log) by hillslope and rep",
   xlab="Hillslope", ylab="Seeds per gram")

boxplot(seeds_per_g_log ~ subsample_g, data=germ_tray, main="Seeds/g (log) by Subsample weight (g)",
   xlab="Subsample Weight (g)", ylab="Seeds per gram")
...
germ_tray_cover <- germ_tray %>% 
  group_by(site, stand_core, nonnative_grass, native_shrub) %>% 
  summarize(sum_seeds_per_g = sum(seeds_per_g), 
            sum_seeds_per_g_log = sum(seeds_per_g_log))

ggqqplot(germ_tray_cover$sum_seeds_per_g_log) 
shapiro.test(germ_tray_cover$sum_seeds_per_g_log) 
# data:  germ_tray_cover$sum_seeds_per_g_log
# W = 0.93109, p-value = 0.08227 <-- normal

summary(aov(sum_seeds_per_g_log ~ site, data = germ_tray_cover))
TukeyHSD(aov(sum_seeds_per_g_log ~ site, data = germ_tray_cover))
Fit: aov(formula = sum_seeds_per_g_log ~ site, data = germ_tray_cover)
# 
# $site
#                   diff        lwr        upr     p adj
# INT2-INT1   3.82415161  -4.284082 11.9323852 0.7136577
# INT3-INT1   1.55304960  -6.555184  9.6612832 0.9948825
# ENH1-INT1   4.20711552  -3.045109 11.4593402 0.5005061
# ENH2-INT1   4.17756615  -3.074658 11.4297908 0.5084042
# DEG1-INT1  -6.92096866 -14.505527  0.6635894 0.0878100 (deg1 - int1)
# DEG2-INT1  -9.76580891 -17.874043 -1.6575753 0.0123601 <--DEG2-INT1 
# INT3-INT2  -2.27110201 -10.379336  5.8371316 0.9645890
# ENH1-INT2   0.38296392  -6.869261  7.6351885 0.9999970
# ENH2-INT2   0.35341454  -6.898810  7.6056392 0.9999981
# DEG1-INT2 -10.74512027 -18.329678 -3.1605622 0.0027451 <--DEG1-INT2 
# DEG2-INT2 -13.58996052 -21.698194 -5.4817269 0.0004389 <--DEG2-INT2 
# ENH1-INT3   2.65406593  -4.598159  9.9062906 0.8848928
# ENH2-INT3   2.62451655  -4.627708  9.8767412 0.8899935
# DEG1-INT3  -8.47401826 -16.058576 -0.8894602 0.0226592 <-- DEG1-INT3  
# DEG2-INT3 -11.31885851 -19.427092 -3.2106249 0.0031818 <--DEG2-INT3  
# ENH2-ENH1  -0.02954938  -6.310160  6.2510614 1.0000000
# DEG1-ENH1 -11.12808419 -17.789678 -4.4664905 0.0004564 <--DEG1-ENH1
# DEG2-ENH1 -13.97292443 -21.225149 -6.7206998 0.0000787 <--DEG2-ENH1
# DEG1-ENH2 -11.09853481 -17.760129 -4.4369411 0.0004708 <--DEG1-ENH2
# DEG2-ENH2 -13.94337506 -21.195600 -6.6911504 0.0000809 <--DEG2-ENH2
# DEG2-DEG1  -2.84484024 -10.429398  4.7397178 0.8730125
```
...
# DEG1 is sig diff from - INT1, INT2, INT3, ENH1, ENH2 
...
# without logged data
seeddensity_nativeshrub <- ggplot(data = germ_tray_cover, 
       aes(x = native_shrub*100, y = sum_seeds_per_g, color = site)) + #color = standtype
...
seeddensity_nativeshrub

summary(lm(sum_seeds_per_g ~ native_shrub, data=germ_tray_cover))
# Residual standard error: 0.4307 on 24 degrees of freedom
# Multiple R-squared:  0.0125,	Adjusted R-squared:  -0.02865 
# F-statistic: 0.3038 on 1 and 24 DF,  p-value: 0.5866 <-- NOT SIG

# with logged data
seeddensity_nativeshrub_LOG <- ggplot(data = germ_tray_cover, 
       aes(x = native_shrub*100, y = sum_seeds_per_g_log, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  # ylim(-10, 0)+
  # xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
  ...
  labs(x = "Native shrub cover (%)", 
       ...
       title = "Seed density (log) by Native Shrub Cover") +
  theme_bw() +  
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2")))
seeddensity_nativeshrub_LOG

ggsave("Ch1_Degradation_figures/seeddensity_nativeshrub_LOG.png", seeddensity_nativeshrub_LOG, width = 8, height = 6)

summary(lm(sum_seeds_per_g_log ~ native_shrub, data=germ_tray_cover))
# Residual standard error: 5.122 on 24 degrees of freedom
# Multiple R-squared:  0.2829,	Adjusted R-squared:  0.2531 
# F-statistic:  9.47 on 1 and 24 DF,  p-value: 0.00516 <-- SIGNIFICANT
```

# seed/g by non-native grass cover
```{r seed density per g x non-native grass cover}
# scatterplot - seed density x nn grass cover
seeddensity_nngrass_LOG <- ggplot(data = germ_tray_cover, 
       aes(x = nonnative_grass*100, y = sum_seeds_per_g_log, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  # ylim(-10,0)+
  # xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
  ...
  labs(x = "Non-native grass cover (%)", 
       y = "Seed density (log(#/g))", 
       title = "Seed density (log) by Non-native Grass Cover") +
  theme_bw() +  
  stat_poly_line() +
   stat_poly_eq(
  ...
  stat_poly_eq(use_label(c("eq", "R2")))
seeddensity_nngrass_LOG

ggsave("Ch1_Degradation_figures/seeddensity_nngrass_LOG.png", seeddensity_nngrass_LOG, width = 8, height = 6)

  seeddensity_nativeshrub_print <- seeddensity_nngrass + theme(legend.position = c(0.1, 0.2)) # Key location offset from (0,0)

summary(lm(sum_seeds_per_g_log ~ nonnative_grass, data=germ_tray_cover))
# Residual standard error: 5.437 on 24 degrees of freedom
# Multiple R-squared:  0.1921,	Adjusted R-squared:  0.1584 
# F-statistic: 5.706 on 1 and 24 DF,  p-value: 0.02512 <-- significant
``` 
...
germ_tray_status_hillslope <- germ_tray_status %>% 
  group_by(site, stand_core, status) %>% 
  summarize(mean_seeds_per_g = mean(seeds_per_g), 
            mean_seeds_per_g_log = mean(seeds_per_g_log),
            mean_native_shrub = mean(native_shrub),
            mean_nonnative_grass = mean(nonnative_grass)
  )
# n = 26 + 26 + 1 unknown

# scatterplot - native seed density x native shrub cover
nativedensity_nativeshrub <- ggplot(germ_tray_status_hillslope %>% 
                                      filter(status == "native"), 
       aes(x = mean_native_shrub*100, y = mean_seeds_per_g_log, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(-10,0)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
  labs(x = "Native shrub cover (%)", 
       y = "Mean native seed density (log(#/g))", 
...
```{r}
germ_tray_NAT <- germ_tray_status %>% 
  filter(status == "native") %>% 
  mutate(site = fct_relevel(site, 
            "INT1", "INT2", "INT3","ENH1", "ENH2", "DEG1", "DEG2")) %>% 
  mutate(treatment = fct_relevel(treatment, 
...
# FAMILY diversity
```{r family diversity}
germ_tray_prep_famdiv <- seedbank %>% 
  group_by(stand_core_depth_treat) %>% 
  filter(count > 0) %>% 
  summarize(unique_Families = length(unique(family)))
glimpse(germ_tray_prep_famdiv) # 205 rows
# stand_core_depth_treat, unique_Families

seedbank_prep_famdiv <-  seedbank %>% 
  group_by(stand, site, soilcore, depth_name, treatment, stand_core, subsample_g, stand_core_depth, stand_core_depth_treat, native_shrub, nonnative_grass) %>% 
  summarize(subsample_g = mean(subsample_g)) 
dim(seedbank_prep_famdiv) #205 rows, 11 columns

germ_tray_famdiv <- germ_tray_prep_famdiv %>% 
  left_join(seedbank_prep_famdiv, by = "stand_core_depth_treat") %>% 
  mutate(Families_per_g = unique_Families/subsample_g,
         Families_per_g_log = log(Families_per_g))
glimpse(germ_tray_famdiv) 
```


```{r Family diversity visualization}

germ_tray_famdiv %>% # NOT TRANSFORMED
  group_by(site) %>% 
  summarize(mean_FamDiv = mean(Families_per_g),
            sd = sd(Families_per_g),
            N = length(Families_per_g), 
            se = ((sd(Families_per_g))/sqrt(length((Families_per_g)))))
# site  mean      SD        N   SE
# DEG1	0.006657593	0.002492672	32	0.0004406464
# DEG2	0.004849358	0.001536726	24	0.0003136828
# ENH1	0.009606148	0.003873929	39	0.0006203252
# ENH2	0.010415403	0.005492513	38	0.0008910032
# INT1	0.016646125	0.008903032	24	0.0018173238
# INT2	0.015518207	0.007406685	24	0.0015118832
# INT3	0.014718954	0.008784949	24	0.0017932202
germ_tray_famdiv %>% # LOG DATA
  group_by(site) %>% 
  summarize(mean_FamDiv = mean(Families_per_g_log),
            sd = sd(Families_per_g_log),
            N = length(Families_per_g_log), 
            se = ((sd(Families_per_g_log))/sqrt(length((Families_per_g_log)))))
# DEG1	-5.095020	0.4430368	32	0.07831859
# DEG2	-5.383196	0.3518011	24	0.07181111
# ENH1	-4.729448	0.4252415	39	0.06809314
# ENH2	-4.711422	0.5781949	38	0.09379559
# INT1	-4.239527	0.5775924	24	0.11790056
# INT2	-4.267890	0.4560354	24	0.09308783
# INT3	-4.401013	0.6434983	24	0.13135354

min(germ_tray_famdiv$unique_Families) #1
max(germ_tray_famdiv$unique_Families) #8
length(germ_tray_famdiv$unique_Families) #205

qplot(site, Families_per_g, data = germ_tray_famdiv)
qplot(native_shrub, Families_per_g, data = germ_tray_famdiv)
qplot(nonnative_grass, Families_per_g, data = germ_tray_famdiv)

qplot(site, Families_per_g, data = germ_tray_famdiv, facets = . ~ depth_name)
qplot(site, Families_per_g, data = germ_tray_famdiv, facets = . ~ treatment)

qplot(native_shrub, Families_per_g, data = germ_tray_famdiv, facets = . ~ depth_name)
qplot(nonnative_grass, Families_per_g, data = germ_tray_famdiv, facets = . ~ depth_name)

boxplot(Families_per_g ~ site, data=germ_tray_famdiv, main="Average Family diversity by Hillslope",
   xlab="Hillslope", ylab="Average Family Diversity (Families/g soil)")
boxplot(Families_per_g_log ~ site, data=germ_tray_famdiv, main="Average Family diversity by Hillslope",
   xlab="Hillslope", ylab="Log Average Family Diversity (Families/g soil)")
```

```{r family diversity by site stats}
ggqqplot(germ_tray_famdiv$Families_per_g) 
shapiro.test(germ_tray_famdiv$Families_per_g) # not transformed
# data:  germ_tray_famdiv$Families_per_g
# W = 0.86159, p-value = 1.075e-12 <-- not normally distributed

ggqqplot(germ_tray_famdiv$Families_per_g_log) 
shapiro.test(germ_tray_famdiv$Families_per_g_log) 
# data:  germ_tray_famdiv$Families_per_g_log
# W = 0.98409, p-value = 0.0206 <-- better but still not normally distributed

kruskal.test(Families_per_g_log ~ site, data = germ_tray_famdiv)
# data:  Families_per_g_log by site
# Kruskal-Wallis chi-squared = 75.364, df = 6, p-value = 3.229e-14
pairwise.wilcox.test(germ_tray_famdiv$Families_per_g_log, 
                     germ_tray_famdiv$site,
                 p.adjust.method = "BH")
# data:  germ_tray_famdiv$Families_per_g_log and germ_tray_famdiv$site 
# 
#      DEG1    DEG2    ENH1   ENH2   INT1   INT2  
# DEG2 0.0061  -       -      -      -      -     
# ENH1 0.0065  4.5e-05 -      -      -      -     
# ENH2 0.0020  6.0e-05 0.6815 -      -      -     
# INT1 1.7e-07 1.7e-07 0.0033 0.0093 -      -     
# INT2 4.5e-08 4.3e-08 0.0049 0.0255 0.6815 -     
# INT3 4.5e-05 5.4e-06 0.0677 0.1463 0.4160 0.6429
# 
# DEG1 - sig diff from other hillslopes
# DEG2 - sig diff from other hillslopes
# ENH1 and ENH2 are similar
# INT1 INT2 and INT3 are similar
# INT3 is similar to ENH1 and ENH2
```

```{r family diversity by soil depth} 
kruskal.test(Families_per_g_log ~ depth_name, data = germ_tray_famdiv)
# data:  Families_per_g_log by depth_name
# Kruskal-Wallis chi-squared = 33.281, df = 1, p-value = 7.974e-09
pairwise.wilcox.test(germ_tray_famdiv$Families_per_g_log, 
                     germ_tray_famdiv$depth_name,
                 p.adjust.method = "BH")
#         deep 
# surface 8e-09

boxplot(Families_per_g_log ~ depth_name, data=germ_tray_famdiv, main="Average Family diversity by Soil Depth",
   xlab="Soil Depth", ylab="Log Average Family Diversity (Families/g soil)")
```

### family diversity x native shrub
```{r family diversity native shrub linear regressions} 
ggplot(data = germ_tray_famdiv, 
       aes(x = native_shrub*100, y = Families_per_g_log, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(-10,0)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
  labs(x = "Native shrub cover (%)", 
       y = "Family Diversity (log(#/g soil))", 
       title = "Family Diversity by Native Shrub Cover") +
  theme_bw() +  
  stat_poly_line() +
   stat_poly_eq(
    aes(label = paste(after_stat(eq.label), after_stat(adj.rr.label), sep = "~~~~")),
...
famdiv_NATshrub.lm = lm(Families_per_g_log ~ native_shrub, 
                                      data = germ_tray_famdiv)
summary(famdiv_NATshrub.lm)
# Residual standard error: 0.536 on 203 degrees of freedom
# Multiple R-squared:  0.2532,	Adjusted R-squared:  0.2495 
# F-statistic: 68.81 on 1 and 203 DF,  p-value: 1.489e-14

```
### family diversity x non-native grass
```{r family diversity nonnative grass linear regressions} 
ggplot(data = germ_tray_famdiv, 
       aes(x = nonnative_grass*100, y = Families_per_g_log, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(-10,0)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
  labs(x = "Non-native grass cover (%)", 
       y = "Family Diversity (log(#/g soil))", 
       title = "Family Diversity by Non-native Grass Cover") +
  theme_bw() +  
  stat_poly_line() +
   stat_poly_eq(
    aes(label = paste(after_stat(eq.label), after_stat(adj.rr.label), sep = "~~~~")),
...
famdiv_NNgrass.lm = lm(Families_per_g_log ~ nonnative_grass, 
                                      data = germ_tray_famdiv)
summary(famdiv_NNgrass.lm)       
# Residual standard error: 0.5253 on 203 degrees of freedom
# Multiple R-squared:  0.2827,	Adjusted R-squared:  0.2792 
# F-statistic: 80.02 on 1 and 203 DF,  p-value: 2.336e-16
```

### familly diversity - how common?
```{r common families}
...
# number of hill slopes (n = 1-7) where each family was observed
view(seedbank %>% 
  group_by(family) %>% 
  summarize(Sites_withfamily = length(unique(site))))
# families found at all seven hill slopes:
# Asteraceae, Fabaceae, Hydrophyllaceae, Poaceae

# number of germ by status
# A)
seedbank %>% 
  group_by(status) %>% 
  summarize(total = sum(count))
...
# B) Can also use df: native_seedbank, nonnative_seedbank
nat_tot_germ <- native_seedbank %>% 
  summarize(total_NATgerm = sum(count)) # = 1052		

nn_tot_germ <- nonnative_seedbank %>% 
  summarize(total_NNgerm = sum(count)) # = 2918
```

```{r native family diversity}
# NATIVE
# total native germ by family
view(seedbank %>% 
  filter(status == "native") %>% 
   group_by(family) %>% 
  summarize(NATgerm_total = sum(count),
            NATgerm_percent = round((NATgerm_total/nat_tot_germ*100), 1))
  )

view(seedbank %>% 
        filter(status == "native") %>% 
       group_by(family) %>% 
       summarize(Sites_withNATfamily = length(unique(site))))


# total native Asters germinated by site
native_seedbank %>% 
    filter(family == "Asteraceae" | family == "Poaceae") %>% #300 germinants
   #group_by(site) %>% 
  summarize(NAT_total = sum(count))) 
  / nat_tot_germ) * 100

seedbank %>% 
  filter(status == "native") %>% 
    filter(family == "Poaceae") %>%  # 168 germinants
   group_by(site) %>% 
  summarize(NAT_total = sum(count))
...
```{r non-native family diversity}
# NON-NATIVE
view(seedbank %>% 
  filter(status == "nonnative") %>% 
   group_by(family) %>% 
  summarize(NNgerm_total = sum(count),
            NNgerm_percent = round((NNgerm_total/nn_tot_germ*100), 1))
  )
# 2466/2918 = 84.5% nonnativegerm were grasses
view(seedbank %>% 
  filter(status == "nonnative") %>% 
   group_by(family) %>% 
 summarize(Sites_withNNfamily = length(unique(site))))

totalNNgerm <- seedbank %>%  
...
totalNNgerm <- sum(nonnative_seedbank$count) #2918

```

# Supplementary Germination Table
```{r supplementary table}
# average total germination (N/g soil)
sup1 <- germ_tray %>%  
  group_by(site) %>% 
  summarize(mean_nativeshrubcover = round(mean(native_shrub)*100, 0),
            mean_nonnativegrasscover = round(mean(nonnative_grass)*100, 0),
            average_germ = round(mean(seeds_per_g),3),
            avggerm_SE = round(((sd(seeds_per_g))/sqrt(length((seeds_per_g)))),3
            ))

# average native germination (N/g soil)
sup2 <- germ_tray_NAT %>% 
  group_by(site) %>% 
   summarize(mean_NATseeddensity = round(mean(seeds_per_g), 3), 
            NAT_se = round(((sd(seeds_per_g))/sqrt(length((seeds_per_g)))),3)
            )

# average nonnative germination (N/g soil)
sup3 <- germ_tray_NN %>% 
  group_by(site) %>% 
   summarize(mean_NNseeddensity = round(mean(seeds_per_g), 3), 
            NN_se = round(((sd(seeds_per_g))/sqrt(length((seeds_per_g)))),3)
            )
view(sup1 %>% 
  left_join(sup2, by = "site") %>% 
  left_join(sup3, by = "site"))
```
