...
twi_firecount_solarrad_load <- read.csv("~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/chaparraldegradation_2022/chaparraldegradation_2022/data/Extracted_Elevation_Slope_Aspect_Hillshade_20m_20241014_8pm.csv", header = TRUE, na.strings = c("", "NA")) %>% 
...
# MEDIANS becuase not all variables are normally distributed
means <- extracted %>%
...
PCA_environcluster <- ggplot() +
  geom_point (data = pca_df,
              aes(x = PC1, 
                  ...
                  color = environ_cluster, 
                  shape = environ_cluster, 
                  fill = environ_cluster), 
              size = 4) +  
  
  # how the data points look
  scale_shape_manual(values = c("1" = 21, "2" = 24, "3" = 22), # circle, triangle, square
                    labels = c("1" = "Sunniest", "2" = "Sunny", "3" = "Shady")) +
 ...
 labs(title = "PCA of Environmental Variables", 
  ...
  xlim(-4, 4) +
  ylim(-5, 4) +
    guides(color = guide_legend(title = "Environmental Group"),
         shape = guide_legend(title = "Environmental Group"),
         fill = guide_legend(title = "Environmental Group"))
...
PCA_plantcluster <- ggplot() +
  ...
  scale_color_manual(values = c("1" = "black", "2" = "black", "3" = "black", "4" = "black", "5" = "black"),
                     labels = c("1" = "NNG", "2" = "ssF", "3" = "ssG", "4" = "ssFG", "5" = "EVG")) +
  scale_fill_manual(values = c("4" = "black", "3" = "grey85", "2" = "grey60", "1" = "white", "5" = "black"),
                     labels = c("1" = "NNG", "2" = "ssF", "3" = "ssG", "4" = "ssFG", "5" = "EVG")) +
 ...
 labs(title = "PCA of Environmental Variables", 
  ...
  xlim(-4, 4) +
  ylim(-5, 4) +
    guides(color = guide_legend(title = "Plant Association"),
         shape = guide_legend(title = "Plant Association"),
         fill = guide_legend(title = "Plant Association"))
...
PCA_nngrass <- ggplot() +
  geom_point (data = pca_df %>% 
                left_join(cover_bylifecycle_wide, by = "site_rep"), # cover_bylifecycle_wide from 1.1_transect_continuous.Rmd
              aes(x = PC1, 
                  y = PC2, 
                  color = grass_annual_nonnative), 
              size = 4) +  
 ...
 labs(title = "PCA of Environmental Variables", 
       x = "Principal Component 1 (40.54%)",
       y = "Principal Component 2 (20.47%)", 
      color = "Non-native annual grass cover (%)") +                  
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))         # Center title
  # xlim(-4, 4) +
  # ylim(-5, 4) 
...
# 5 0.0053 0.0053 0.0053 0.0053 5 = c
# 
...
### - stats
```{r}
# Here I'm prepping the data: df = env_vars
env_vars <- boxdf[, c("slope", "southwestness", "total_solrad", "twi")]

# Here I'm running a PERMANOVA
adonis2(env_vars ~ plant_cluster, data = boxdf, method = "euclidean", permutations = 999)
#          Df   SumOfSqs      R2     F Pr(>F)    
# Model     4 1369471011 0.61746 13.72  0.001 *** <-- sig - these plant clusters did not occur by chance. 
# Residual 34  848421755 0.38254                 
# Total    38 2217892766 1.00000  
# Output: plant_cluster explains 61.7% of environmental variation, p = 0.001

# Here is an ordinations to visualize the data - PCA
pca_env <- rda(env_vars) # PCA on environmental variables

# Fit vectors to see which environmental variables and clusters correlate with PCA axes
ef_vars <- envfit(pca_env, env_vars, permutations = 999) # Environmental variable vectors
# Here I'm figuing out which clusters correlate with environmental gradients
ef_clusters <- envfit(pca_env, boxdf$plant_cluster, permutations = 999)

# plot PCA with vectors - run together
plot(pca_env, type = "n")  # empty canvas
points(pca_env, display = "sites", col = boxdf$plant_cluster, pch = 16) # points colored by plant cluster
plot(ef_vars, col = "blue") # overlay envfit vectors; environmental variable arrows
plot(ef_clusters, col = "red")   # cluster centroid arrows

# - Optional: add ellipses around clusters
# ordiellipse(pca_env, boxdf$plant_cluster, kind = "sd", conf = 0.95, label = TRUE)
par(cex.axis = 1.5, cex.lab = 1.5, cex.main = 1.8)  # increase overall text

# Empty PCA canvas
plot(pca_env, type = "n", main = "PCA of Environmental Variables")

# Points colored by cluster, bigger size
points(pca_env, display = "sites", col = boxdf$plant_cluster, pch = 16, cex = 1.5)

# Overlay environmental vectors (blue)
plot(ef_vars, col = "blue", cex = 1.2, length = 0.1)

# Overlay cluster centroid vectors (red)
plot(ef_clusters, col = "black", cex = 1.2, length = 0.1)
...
# Fig. Salvia cover x Total Solar Radiation
...
pca_data_woINT <- extracted %>%
  dplyr::select(c(slope, twi, southwestness, fire_event_count, site_rep, site)) %>% # <- omit hillshade
