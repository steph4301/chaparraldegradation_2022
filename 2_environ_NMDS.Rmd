...
### TABLE means
- environmental means by plant cluster
```{r}
...
means <- extracted %>%
   dplyr::select(c("site_rep", "plant_cluster", 
                   ...
                   "summer_solarradiation", "winter_solarradiation", "equinox_solarradiation", "total_solrad")) %>%
  ...
  summarize(
    ...
    summer_solrad = paste0(round(mean(summer_solarradiation), 0), " ± ", round(sd(summer_solarradiation), 0)),
      winter_solrad = paste0(round(mean(winter_solarradiation), 0), " ± ", round(sd(winter_solarradiation), 0)),
      equinox_solrad = paste0(round(mean(summer_solarradiation), 0), " ± ", round(sd(summer_solarradiation), 0)),
       total_solrad = paste0(round(mean(total_solrad), 0), " ± ", round(sd(total_solrad), 0))
  ) 
...
table_plant <- means %>%
  ...
  tab_header(
    ...
    subtitle = "Mean ± Standard Deviation of Environmental Variables"
  ) %>%
  ...
  cols_label(
    ...
    summer_solrad = "Summer Solstice", 
    winter_solrad = "Winter Solstice",
    equinox_solrad = "Equinox", 
...
# Scree plot  
fviz_nbclust(df, FUN = hcut, diss = d, method = "wss") # 2, 3, or 4 groups
...
fviz_nbclust(df, FUN = hcut, diss = d, method = "gap_stat") # 9 groups, there's also a break at 3 groups
...
pca_data <- extracted %>% 
  dplyr::select(c(slope, twi, southwestness, fire_event_count, site_rep, summer_solarradiation, winter_solarradiation, equinox_solarradiation)) %>% 
  column_to_rownames("site_rep")
...
# Standard deviation     1.5361 1.3759 1.0256 0.8627 0.70418 0.67483 0.00746
# Proportion of Variance 0.3371 0.2705 0.1502 0.1063 0.07084 0.06506 0.00001
# Cumulative Proportion  0.3371 0.6075 0.7578 0.8641 0.93494 0.99999 1.00000
summary(pca_scores)
...
PCA_plot <- ggplot() +
  ...
  labs(title = "PCA of Environmental Variables", 
       x = "Principal Component 1 (33.71%)",
       y = "Principal Component 2 (27.05%)") +                  
  #clean_background +      
  theme_bw()+
