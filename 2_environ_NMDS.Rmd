...
#    summarize(hill_mean = mean(hillshade),
...
#            twi_sd = sd(twi)
#             ) %>% 
#   pivot_longer(cols = 2:11, values_to = "mean", names_to = "variable") %>% 
...
means_combined <- df %>%
  ...
  summarize(
    ...
    twi = paste0(round(mean(twi), 2), " ± ", round(sd(twi), 2))
  ) 

#%>% 
    mutate(plant_cluster = case_when(plant_cluster == "1" ~ "NNG", # old ~ new
                                   plant_cluster == "2" ~ "ssF",
                                   plant_cluster == "3" ~ "ssG",
                                  plant_cluster == "4" ~ "ssFG",
                                    plant_cluster == "5" ~ "EVG",
                                   TRUE ~ plant_cluster)) 
...
table_plant <- means_combined %>%
  ...
  tab_header(
    ...
    subtitle = "Mean ± Standard Deviation for Hillshade, Slope, Southwestness, Fire Events, and Topographic Wetness Index"
  ) %>%
  # Rename the column headers
  cols_label(
    plant_cluster = "Plant Group",        # Rename "plant_cluster" column
    hillshade = "Hillshade",                # Rename "hillshade" column
    slope = "Slope (%)",                    # Rename "slope" column
    southwestness = "Southwestness",        # Rename "southwestness" column
    fire_events = "Number of Fires",        # Rename "fire_events" column
    twi = "Topographic Wetness Index"       # Rename "tpi" column
  ) %>%
  cols_align(
...
df <- df %>% # from two chunks above
  dplyr::select(hillshade, slope, southwestness, twi, fire_event_count, plant_cluster)
...
# Best method is g average at 0.9650463 
...
# $group
#             diff         lwr       upr     p adj
# 2-1 -0.001990316 -0.02405513 0.0200745 0.8559771
...
gof
# min(gof) = 0.004066253
# max(gof) = 0.008725043 <- relationships in your data are well represented!
...
pca_result <- prcomp(scaled_columns, center = TRUE, scale = TRUE)
...
pca_df$Environmental_Cluster <- pca_data$environ_cluster  # add column for clustering
pca_df$Plant_Cluster <- pca_data$plant_cluster  # add column for clustering
pca_df <- pca_df %>% 
    mutate(Environmental_Cluster = as.character(Environmental_Cluster)) %>% 
   mutate(Plant_Cluster = as.character(Plant_Cluster))


PCA_plot <- ggplot() +
  geom_point (data = pca_df,
              aes(x = PC1, y = PC2, color = Environmental_Cluster, shape = Plant_Cluster), 
              size = 4) +  
  scale_color_manual(values = c("1" = "orange", "2" = "darkgreen")) +
  ...
  labs(title = "PCA of Environmental Variables", 
       x = "Principal Component 1 (35.64%)",
       y = "Principal Component 2 (23.53%)") +                  
  #clean_background +      
  theme_bw()+
...
PCA_environcluster <- ggplot() +
  geom_point (data = pca_df,
              aes(x = PC1, y = PC2, color = Environmental_Cluster, shape = Environmental_Cluster), 
              size = 4) +  
  scale_color_manual(values = c("1" = "orange", "2" = "darkgreen")) +
  #scale_shape_manual(values = c("1" = 0, "2" = 4, "3" = 2, "4" = 3, "5" = 7)) +
  geom_vline(xintercept = c(0), color = "grey70", linetype = 2) + 
...
PCA_plantcluster <- ggplot() +
  geom_point (data = pca_df,
              aes(x = PC1, y = PC2, color = Plant_Cluster, shape = Plant_Cluster), 
              size = 4) +  
scale_color_manual(values = c("1" = "black", "2" = "red", "3" = "green", "4" = "blue", "5" = "cyan"),
                     labels = c("1" = "NNG", "2" = "SS+F", "3" = "SS+G", "4" = "SS+FG", "5" = "EVG")) +
...
PCA_environcluster_deg

PCA_plantcluster_deg
...
hist(boxdf$elevation)
shapiro.test(boxdf$elevation) # not normal

kruskal.test(elevation ~ environ_cluster, data = boxdf)
pairwise.wilcox.test(boxdf$elevation, boxdf$environ_cluster, p.adjust.method = "BH")
#   1      
# 2 3.2e-07

boxplot(elevation ~ environ_cluster, boxdf)


########################################## hillshade
hist(boxdf$hillshade)
shapiro.test(boxdf$hillshade) # normally distributed

summary(aov((hillshade ~ environ_cluster), data = boxdf))
#                Df Sum Sq Mean Sq F value Pr(>F)  
# environ_cluster  1   1450  1450.1   4.249 0.0463 *
# Residuals       37  12627   341.3                      

TukeyHSD(aov(hillshade ~ environ_cluster, boxdf), conf.level=.95) 
# $environ_cluster
#         diff       lwr      upr     p adj
# 2-1 12.71143 0.2168144 25.20604 0.0463482 <- sig

boxplot(hillshade ~ environ_cluster, boxdf)
...
kruskal.test(southwestness ~ environ_cluster, data = boxdf) # p-value = 0.6818
...
pairwise.wilcox.test(boxdf$twi, boxdf$environ_cluster, p.adjust.method = "BH")
...
summary(aov(slope ~ environ_cluster, boxdf))
TukeyHSD(aov(slope ~ environ_cluster, boxdf), conf.level = .95) # not sig diff
# $environ_cluster
#          diff       lwr      upr     p adj
...
########################################## lat
hist(boxdf$lat)
shapiro.test(boxdf$lat) # not normal

kruskal.test(lat ~ environ_cluster, data = boxdf) 
# Kruskal-Wallis chi-squared = 13.181, df = 1, p-value = 0.0002828
pairwise.wilcox.test(boxdf$lat, boxdf$environ_cluster, p.adjust.method = "BH")

boxplot(lat ~ environ_cluster, boxdf)

########################################## long
hist(boxdf$long)
shapiro.test(boxdf$long) # not normal

kruskal.test(long ~ environ_cluster, data = boxdf) 
pairwise.wilcox.test(boxdf$long, boxdf$environ_cluster, p.adjust.method = "BH")
#  1     
# 2 0.0022

boxplot(long ~ environ_cluster, boxdf)
...
hist(boxdf$elevation)
shapiro.test(boxdf$elevation)
# Shapiro-Wilk normality test
# 
# data:  df$elevation
# W = 0.88322, p-value = 0.0007544 <-- not normally distributed

kruskal.test(elevation ~ plant_cluster, data = boxdf)
pairwise.wilcox.test(df$elevation, boxdf$plant_cluster, p.adjust.method = "BH")
...
boxplot(elevation ~ plant_cluster, boxdf)
## 2 is diff from 1 and 3

########################################## hillshade
hist(boxdf$hillshade)
shapiro.test(boxdf$hillshade)
# Shapiro-Wilk normality test
# 
# data:  df$hillshade
# W = 0.97317, p-value = 0.467 <-- normally distributed
summary(aov((hillshade ~ plant_cluster), data = boxdf))
#  Df Sum Sq Mean Sq F value Pr(>F)   
# cluster      4   4559  1139.7   4.071 0.0084 **
# Residuals   34   9518   279.9                  

TukeyHSD(aov(hillshade ~ plant_cluster, boxdf), conf.level=.95) 
# $cluster
#           diff         lwr        upr     p adj
...
boxplot(hillshade ~ plant_cluster, boxdf)
...
hist(boxdf$southwestness)
shapiro.test(boxdf$southwestness) # <- not normal
kruskal.test(southwestness ~ plant_cluster, data = boxdf)
...
########################################## lat
boxplot(lat ~ plant_cluster, boxdf)

########################################## long
boxplot(long ~ plant_cluster, boxdf)
...
############ summary table by environmental cluster 
...
means_2 <- boxdf %>% 
  ...
  pivot_wider(id_cols = environ_cluster, names_from = variable, values_from = mean)

means_combined_2 <- boxdf %>%
  group_by(environ_cluster) %>%
  summarize(
