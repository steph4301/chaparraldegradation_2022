...
extracted_load <- read.csv("~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/chaparraldegradation_2022/chaparraldegradation_2022/Extracted_Elevation_Slope_Aspect_Hillshade_20m.csv", header = TRUE, na.strings=c("","NA")) %>% 
  dplyr::select(system_ind, aspect, elevation, hillshade, slope, site, lat, long, cluster) %>% 
...
means <- extracted %>%
   dplyr::select(c("plant_cluster", "hillshade", "slope", "southwestness", "twi", "fire_event_count", "site_rep")) %>%
...
df <- extracted %>%  
  ...
  dplyr::select(c("hillshade", "slope", "southwestness", "twi", "fire_event_count"))
...
pca_data <- extracted %>% 
  dplyr::select(c(hillshade, slope, twi, southwestness, fire_event_count, site_rep)) %>% 
  column_to_rownames("site_rep")
...
PCA_plot <- ggplot() +
  ...
  labs(title = "PCA of Environmental Variables", 
       x = "Principal Component 1 (41.67%)",
       y = "Principal Component 2 (23.10%)") +                  
  #clean_background +      
  theme_bw()+
