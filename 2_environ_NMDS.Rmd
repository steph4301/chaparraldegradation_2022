...
exclude_names <- c("site_rep", "site", "plant_cluster", "rowname", "last_fire", "environ_cluster")
...
scaled_columns <- df %>%
  dplyr::select(-all_of(exclude_columns)) %>%  # All other columns (to be scaled)
...
pca_df$Environmental_Cluster <- df$environ_cluster  # add column for clustering
pca_df$Plant_Cluster <- df$plant_cluster  # add column for clustering
...
plot_PCA <- ggplot() +
  ...
  labs(title = "PCA of Environmental Variables", 
       x = "Principal Component 1 (31.86%)",
       y = "Principal Component 2 (21.14%)") +                  
  #clean_background +      
  theme_bw()+
