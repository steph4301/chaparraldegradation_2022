...
cover_wide <- cover_byleaftype_wide %>% 
  left_join(cover_bylfstatus_wide, by = "site_rep", "site")
...
```{r}
fig.1b <- ggplot(data = (cover_byleaftype_wide %>%  
...
# Update letters for correct format
letters <- significance_letters %>%
  dplyr::select(cluster = group1, label)
...
nngrass_cluster <- ggplot(cover_bylifecycle_bygroup %>%
                            filter(status == "nonnative") %>% 
         filter(lifeform_lifecycle %in% c("grass_annual")),
       aes(x = cluster, 
           y = mean_cover, 
           shape = lifeform,   # Shape by lifeform
           color = lifecycle   # Color by lifecycle
  ...
  geom_errorbar(aes(ymin = mean_cover - se, ymax = mean_cover + se), 
                width = 0.2, position = position_dodge(width = 0.3)) +  # SE bars
  labs(x = "Plant Cluster", 
       y = "Mean Alien Annual Grass Percent Cover (%)", 
       color = "Lifecycle",  # Label for color legend
       shape = "Lifeform") +  # Label for shape legend
  theme_bw() +
  theme(
  ...
  ylim(0, 100) +
 # facet_wrap(~lifeform) +  # Facet by lifeform
  scale_shape_manual(values = c("grass" = 16)) +  
  scale_color_manual(values = c("annual" = "black")) +  
  scale_x_discrete(labels = c("1" = "NNG", "2" = "ssF", "3" = "ssG", "4" = "ssFG", "5" = "EVG"))  # Relabel clusters
...
densitymeans_bycluster <- density_cover_cluster %>% # without shrub species
  ...
  dplyr::summarise(
   ...
   sd = sd(sum_density_m2, na.rm = TRUE), 
    se = sd / sqrt(n),
    mean_sd = paste0(round(mean_density, 2), " ± ", round(sd, 2))  # New column for formatted mean ± sd
  ) 

    
densitymeans_bycluster_TABLE <- densitymeans_bycluster %>% 
  dplyr::select(cluster, lifestage, mean_sd) %>%  # Select relevant columns for the summary table
  pivot_wider(names_from = lifestage, values_from = mean_sd) %>% 
  mutate(cluster = case_when(cluster == "1" ~ "NNG",
                             cluster == "3" ~ "ssG",
                             cluster == "2" ~ "ssF",
                             cluster == "4" ~ "ssFG",
                             cluster == "5" ~ "EVG",
                             TRUE ~ cluster)) %>% 
  relocate(immature, mature, dead, .after = seedling) %>% 
  dplyr::select(-no_shrubs) %>% 
 # kable(caption = "Summary Table of Means ± SD for Each Cluster and Lifestage")
  gt() %>%
  tab_header(
    title = "Mean Percent Shrub Density by Lifestage and Cluster",
