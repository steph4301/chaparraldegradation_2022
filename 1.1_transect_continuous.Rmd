...
model.scl <- lm(shrub_native_sclerophyllous ~ grass_nonnative_NA, data = (cover_byleaftype_wide
                                                                          # %>%  filter(shrub_native_sclerophyllous > 0)
...
######### sage scrub slope
# Fit the linear model
model.ss <- lm(shrub_native_nonsclero ~ grass_nonnative_NA, data = (cover_byleaftype_wide )) # response variable ~ independent variable
...
fig.1a <- ggplot(data = cover_byleaftype_wide) +
  ...
  geom_point(data = cover_byleaftype_wide 
             ...
             size = 3, show.legend = TRUE) +  # Adjust point size here
  ...
  # Sage Scrub Cover
  geom_point(data = cover_byleaftype_wide
             #%>% filter(shrub_native_nonsclero > 0)
             ,
  ...
  scale_shape_manual(values = c("sclerophyllous" = 7, "sage scrub" = 16)) +  # Square = 15, Circle = 16
...
# Fit the linear model
model.g <- lm(grass_native ~ grass_nonnative, data = cover_bylfstatus_wide 
...
fig_forbgrass <- ggplot(cover_bylifecycle_bygroup %>%
  ...
  labs(x = "Plant Cluster", 
       ...
       shape = "Lifecycle",  # Label for color legend
       color = "Status") +  # Label for shape legend
  theme_bw() +
  theme(
  ...
  scale_shape_manual(values = c("perennial" = 16, "annual" = 6)) +  
  scale_color_manual(values = c("nonnative" = "orange", "native" = "darkgreen")) +  
  scale_x_discrete(labels = c("1" = "NNG", "2" = "ssF", "3" = "ssG", "4" = "ssFG", "5" = "EVG"))  # Relabel clusters
fig_forbgrass

ggplot(cover_bylifecycle_bygroup %>%
...
sppbygroup <- denspp %>% 
  ...
  summarize(mean_m2 = round(mean(density_m2),2)) %>% 
  pivot_wider(names_from = cluster, values_from = mean_m2) 
  
view(sppbygroup)

shrubs_evg <- sppbygroup %>% 
  dplyr::select(species, lifestage, "5") %>% 
  filter("5" > 0) %>% 
  filter(lifestage == "seedling")
view(shrubs_evg)

##### bind WITHOUT shrub species
...
######################################## calculate means by cluster (n = 40 transects)
...
densitymeans_bycluster <- density_cover_cluster %>% 
  mutate(cluster = factor(cluster, levels = c(1, 3, 2, 4, 5))) %>% 
  group_by(cluster, lifestage) %>% 
  dplyr::summarise(
    mean_density = mean(sum_density_m2, na.rm = TRUE), 
    n = n_distinct(site_rep), 
    sd = sd(sum_density_m2, na.rm = TRUE), 
    se = sd / sqrt(n)
  ) 
  
densitymeans_bycluster_table <- densitymeans_bycluster %>%
  group_by(cluster) %>% 
  mutate(cluster = factor(cluster, levels = c(1, 3, 2, 4, 5))) %>%  # Reorder plant_cluster
  summarize(
    seedling = paste0(round(mean(mean_density), 2), " ± ", round(sd(mean_density), 2)),
    immature = paste0(round(mean(mean_density), 2), " ± ", round(sd(mean_density), 2)),
    mature = paste0(round(mean(mean_density), 2), " ± ", round(sd(mean_density), 2)),
    dead = paste0(round(mean(mean_density), 2), " ± ", round(sd(mean_density), 2))
  ) %>% 
  mutate(seedling_sig = c("a", "ab", "b", "b", "ab")) %>% 
  mutate(immature_sig = c("a", "a", "a", "a", "a")) %>% 
  mutate(mature_sig = c("a", "ab", "ab", "b", "ab")) %>% 
  mutate(dead_sig = c("a", "ac", "bc", "ac", "bc")) %>% 
  # Add superscripts for significance levels for visualization
  mutate(
    seedling = html(paste0(seedling, "<sup>", seedling_sig, "</sup>")),
    immature = html(paste0(immature, "<sup>", immature_sig, "</sup>")),
    mature = html(paste0(mature, "<sup>", mature_sig, "</sup>")),
    dead = html(paste0(dead, "<sup>", dead_sig, "</sup>"))
  )
...
# Print as a gt table
table_shrubden <- densitymeans_bycluster_table %>%
  ...
  gt() %>%
  tab_header(
    title = "Shrub Density by Lifestage and Plant Cluster",
    subtitle = "Mean ± Standard Deviation"
  ) %>%
  # Rename the column headers
  cols_label(
    cluster = "Plant Group",        # rename plant_cluster column
    seedling = "Seedling",                
    immature = "Immature",      
    mature = "Mature",       
    dead = "Dead"
  ...
  cols_align(
    ...
    columns = "cluster" # Specify the "cluster" column
  ) %>% 
  tab_options(
    table.width = pct(100) # Make the table width 100%
  ) #%>% 
  #gtsave("shrub_density_table2.png")

table_shrubden
  

# Create the table with superscripts
densitymeans_with_superscript <- densitymeans_bycluster %>%
  group_by(cluster) %>%
  mutate(cluster = factor(cluster, levels = c(1, 3, 2, 4, 5))) %>%
  summarize(
    seedling = paste0(round(mean(mean_density), 2), " ± ", round(sd(mean_density), 2)),
  ...
  mutate(
    seedling_sig = c("a", "ab", "b", "b", "ab"),
    immature_sig = c("a", "a", "a", "a", "a"),
    mature_sig = c("a", "ab", "ab", "b", "ab"),
    dead_sig = c("a", "ac", "bc", "ac", "bc")
  ) %>%
  # Add superscripts for significance levels for visualization
  mutate(
    seedling = html(paste0(seedling, "<sup>", seedling_sig, "</sup>")),
...
# Print the table as a gt table
densitymeans_with_superscript %>%
  gt() %>%
  tab_header(
    title = "Shrub Density by Lifestage and Plant Cluster",
    subtitle = "Mean ± Standard Deviation"
  ...
  cols_align(align = "center", columns = "cluster") %>%
  tab_options(table.width = pct(100))

############################## density stats among groups
...
# 2 0.0319 -      -      -     
# 3 0.8217 0.6118 -      -     
# 4 0.0031 0.8217 0.0810 -     
# 5 0.2115 0.8534 0.8534 0.8217
# 
...
pairwise.wilcox.test(density_immature$sum_density_m2, 
                                      ...
                                      p.adjust.method = "holm") # <- no sig
# with "holm" adjusted p value
#   1    2    3    4   
# 2 0.26 -    -    -   
# 3 1.00 0.43 -    -   
# 4 0.78 1.00 0.87 -   
# 5 0.18 1.00 0.49 1.00

# data:  density_immature$sum_density_m2 and density_immature$cluster - with bonferroni adjused p-value
# 
#   1    2    3    4   
# 2 0.29 -    -    -   
# 3 1.00 0.54 -    -   
...
# data:  sum_density_m2 by cluster
# Kruskal-Wallis chi-squared = 18.346, df = 4, p-value = 0.001056 <-- sig
...
# 2 0.0113 -      -      -     
# 3 0.5026 0.0792 -      -     
# 4 0.0685 0.5026 0.5026 -     
# 5 0.0095 1.0000 0.2132 0.8748
# 
# P value adjustment method: holm 
