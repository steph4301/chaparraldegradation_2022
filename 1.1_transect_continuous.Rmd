...
fig.1a <- ggplot(data = cover_byleaftype_wide) +
  ...
  geom_smooth(data = cover_byleaftype_wide %>% filter(shrub_native_sclerophyllous > 0), 
              ...
              method = "lm", se = TRUE, linetype = "dashed", show.legend = TRUE) +
  # Sage Scrub Cover
  geom_point(data = cover_byleaftype_wide %>% filter(shrub_native_nonsclero > 0), 
             aes(x = grass_nonnative_NA * 100, y = shrub_native_nonsclero * 100, 
                 color = "sage scrub", shape = "sage scrub"), 
             size = 3, show.legend = TRUE) +  # Adjust point size here
  geom_smooth(data = cover_byleaftype_wide %>% filter(shrub_native_nonsclero > 0), 
...
cover_bylifecycle <- abcover_omitground %>% 
  ...
  group_by(year, site, transect, site_rep, lifeform, lifecycle, lifeform_lifecycle, cluster) %>%
  dplyr::summarize(
...
cover_bylifecycle_bygroup <- cover_bylifecycle %>% 
  group_by(cluster, lifeform, lifecycle, lifeform_lifecycle) %>% 
  dplyr::summarise(
...
ggplot(cover_bylifecycle_bygroup %>%
         filter(lifeform_lifecycle %in% c("forb_annual", "grass_annual", "forb_perennial", "grass_perennial")),
       aes(x = cluster, 
           y = mean_cover, 
  ...
  scale_shape_manual(values = c("forb" = 16, "grass" = 16)) +  # Square for forb, circle for grass
  scale_color_manual(values = c("annual" = "grey70", "perennial" = "black")) +  # Orange for annual, green for perennial
...
shrub_perennial_data <- cover_bylifecycle %>%
  filter(lifeform_lifecycle == "shrub_perennial")

hist(shrub_perennial_data$lifeform_lifecycle_percentcover)
shapiro.test(shrub_perennial_data$lifeform_lifecycle_percentcover)
# data:  shrub_perennial_data$lifeform_lifecycle_percentcover
# W = 0.94181, p-value = 0.06355 <-- normal

aov(lifeform_lifecycle_percentcover ~ cluster, data = shrub_perennial_data)
summary(aov(lifeform_lifecycle_percentcover ~ cluster, data = shrub_perennial_data))
# Df Sum Sq Mean Sq F value   Pr(>F)    
# cluster      4  14974    3743   14.48 1.08e-06 ***
# Residuals   30   7755     258                     
tukey_result <- TukeyHSD(aov(lifeform_lifecycle_percentcover ~ cluster, data = shrub_perennial_data), conf.level=.95) 
# $cluster
#         diff        lwr      upr     p adj
...
judge$letter <- multcompLetters(tukey_df[,4, drop = FALSE], compare = "<", threshold = 0.05)$letter




# Create a new column with significant letters
judge$letter <- multcompLetters(tukey_df[,4, drop = FALSE], 
                                    compare = "<", 
                                    threshold = 0.05)$letter


kruskal.test(lifeform_lifecycle_percentcover ~ cluster, data = shrub_perennial_data)
