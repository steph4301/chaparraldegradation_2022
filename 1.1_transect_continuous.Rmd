...
# breakpoint = 52.7%
# slope < 52.7% = -0.35924; p < 0.001
# slope > 52.7% = -1.22714; p = 0.223  
...
nngxantgrass <- ggplot(data = cover_bylfstatus_wide,
  ...
  theme(legend.title=element_text(size = 16)) + 
  theme(legend.text=element_text(size = 16))+
  annotate("text", x = 60, y = 90, label = label_g, color = "black", size = 4, hjust = 0) 
...
nngxnatherb <- ggplot(data = herb_cover_wide,
  ...
  theme(legend.title=element_text(size = 16)) + 
  theme(legend.text=element_text(size = 16)) +
   annotate("text", x = 0, y = 90, label = label_h, color = "black", size = 4, hjust = 0) 
...
# To decide if non-linear/threshold models are “better” than linear:
# Compare models using AIC:
AIC_herb <- (AIC(model_lin, model_poly, model_gam, model_seg))
# Lower/more negative AIC = better fit.
...
#               df       AIC
# model_lm_bg    3 -42.71115 <-- lowest number = "best"
# model_poly_bg  4 -41.08067
# model_seg_bg   5 -42.42147
...
hist(groundcover_percent$bare_ground)
shapiro.test(groundcover_percent$bare_ground) # p-value = 0.00199 <- not normal

cor.test(groundcover_percent$grass_nonnative, 
         groundcover_percent$bare_ground, method = "spearman") 
...
# meaning --> There is a weak negative correlation (ρ = –0.297) between non-native grass cover and bare ground, but the relationship was not statistically significant (p = 0.063).


# linear regression
model_lm_bg <- lm(bare_ground ~ grass_nonnative, data = groundcover_percent) 
    # (Intercept)  grass_nonnative  
    #      0.2544          -0.1109 

# Polynomial Regression (Curvilinear Trends) - Fit a model where shrub cover changes in a curve (concave or convex) with grass cover.
model_poly_bg <- lm(bare_ground ~ poly(grass_nonnative, 2), data = groundcover_percent)
summary(model_poly_bg)
# Coefficients:
#                           Estimate Std. Error t value Pr(>|t|)    
# (Intercept)                0.18232    0.02154   8.465 3.51e-10 ***
# poly(grass_nonnative, 2)1 -0.21416    0.13622  -1.572    0.124    
# poly(grass_nonnative, 2)2 -0.07983    0.13622  -0.586    0.561    
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 0.1362 on 37 degrees of freedom
# Multiple R-squared:  0.0707,	Adjusted R-squared:  0.02047 
# F-statistic: 1.408 on 2 and 37 DF,  p-value: 0.2575
...
# Coefficients of the linear terms:
#                    Estimate Std. Error t value Pr(>|t|)
# (Intercept)         0.02439    0.13245   0.184    0.855
# grass_nonnative     3.49024    2.21702   1.574    0.124
# U1.grass_nonnative -3.64763    2.21857  -1.644       NA
# 
# Residual standard error: 0.1325 on 36 degrees of freedom
# Multiple R-Squared: 0.1452,  Adjusted R-squared: 0.07393 
# 
