...
Fstatistics <- model.s_summary$fstatistic
Fstat = round(Fstatistics[1], 2)
on = round(Fstatistics[2], 0)
DF = round(Fstatistics[3], 0)
r_squareds <- round(model.s_summary$r.squared, 3)
print(paste("R^2:", r_squareds))

# Create the label for the equation and R^2
label_s <- paste0("Second-order polynomial regression", "\n",
                  "F(", on, ",", DF,") = ", Fstat, "\n", 
                  "p < 0.001", "\n", 
                  "R² = ", r_squareds)
...
nngxnatshr <- ggplot(data = cover_bylfstatus_wide, 
  ...
  annotate("text", x = 0, y = 10, label = label_s, color = "black", size = 4, hjust = 0)
...
summary(
  segmented(model_lm, seg.Z = ~grass_nonnative,
                       npsi = 1) 
)
# Est. St.Err
...
AICshrub <- (AIC(model_lm, model_poly, model_gam, model_seg))
...
herb_cover_wide_transformed <- herb_cover_wide %>% 
  mutate(log10_herb_native = log10(herb_native + 1)) %>% 
  mutate(log1p_herb_native = log1p(herb_native)) %>% 
  mutate(across(c(log10_herb_native, log1p_herb_native), ~na_if(., 0))) %>% 
  dplyr::select(site_rep, herb_native, log10_herb_native, log1p_herb_native)
...
# Fit the model
model.f <- lm(forb_native ~ poly(grass_nonnative, 2), data = cover_bylfstatus_wide)
model.f.summary <- summary(model.f)

Fstatistics <- model.f.summary$fstatistic
Fstat = round(Fstatistics[1], 2)
on = round(Fstatistics[2], 0)
DF = round(Fstatistics[3], 0)
r_squaredf <- round(model.f.summary$r.squared, 3)
print(paste("R^2:", r_squaredf))

# Create the label for the equation and R^2
label_f <- paste0("Second-order polynomial regression", "\n",
                  "F(", on, ",", DF,") = ", Fstat, "\n", 
                  "p = 0.264", "\n", # <-- p-value
                  "R² = ", r_squaredf)
...
nngxnatforb <- ggplot(data = cover_bylfstatus_wide,
  ...
  theme(legend.title=element_text(size = 16)) + 
  theme(legend.text=element_text(size = 16))+
  annotate("text", x = 0, y = 90, label = label_f, color = "black", size = 4, hjust = 0)
...
# Fit the model
model.natg <- lm(grass_native ~ poly(grass_nonnative, 2), data = cover_bylfstatus_wide)
model.natg_summary <- summary(model.natg)

Fstatistics <- model.natg_summary$fstatistic
Fstat = round(Fstatistics[1], 2)
on = round(Fstatistics[2], 0)
DF = round(Fstatistics[3], 0)
r_squaredf <- round(model.natg_summary$r.squared, 3)
print(paste("R^2:", r_squaredf))

# Create the label for the equation and R^2
label_natg <- paste0("Second-order polynomial regression", "\n",
                  "F(", on, ",", DF,") = ", Fstat, "\n", 
                  "p < 0.001", "\n", # <-- p-value
                  "R² = ", r_squaredf)
...
nngxantgrass <- ggplot(data = cover_bylfstatus_wide,
  ...
  theme(legend.title=element_text(size = 16)) + 
  theme(legend.text=element_text(size = 16))+
  annotate("text", x = 60, y = 90, label = label_natg, color = "black", size = 4, hjust = 0) 
...
# 4. Generalized Additive Models (GAMs) - If you don’t want to assume the shape (e.g., parabola), GAMs let the data “decide” the curve:
model_gam <- gam(herb_native ~ s(grass_nonnative), data = herb_cover_wide)
summary(model_gam)
plot(model_gam)
...
# 6. Quantile Regression - Sometimes the average trend isn’t the full story — for example, shrubs may persist under moderate grass cover only in the “best-case” sites. Quantile regression shows how the relationship differs across the distribution of shrub cover.
model_qr <- rq(herb_native ~ grass_nonnative, tau = 0.9, data = herb_cover_wide)
summary(model_qr)
...
fig.1b_labeled <- ggdraw(nngxnatforb) + draw_label("B", x = 0.95, y = 0.92, size = 15)  # nat forb
fig.1c_labeled <- ggdraw(nngxantgrass) + draw_label("C", x = 0.95, y = 0.92, size = 15)  # nat grass
...
fig_height <- ggplot(data = height_cover %>% 
       mutate(site = ifelse(str_detect(site, "INT"), "3", site)) %>% 
         dplyr::select(site, site_rep, grass_nonnative, avgheight_cm) %>% 
         distinct(), 
...
# To decide if non-linear/threshold models are “better” than linear:
# Compare models using AIC:
(AIC(model_lin, model_poly, model_seg)) # Best models: GAM, segmented
# Lower/more negative AIC = better fit.
...
fig_clusterheight_scatter <- ggplot(data = height_cover %>% 
       mutate(site = ifelse(str_detect(site, "INT"), "3", site)) %>% 
         dplyr::select(site, cluster, grass_nonnative, avgheight_cm) %>% 
         distinct() %>% 
          mutate(cluster = case_when(
                            cluster == "1" ~ "NNG",
                             cluster == "3" ~ "ssG",
...
ggplot(transect_cov,
       aes(x = cluster,
           ...
           fill = cluster) ) +
  barplot() +
  scale_fill_manual(values = colors_plantclusters)
...
fig_den_mature <- ggplot(denspp_cover %>% 
         mutate(site = ifelse(str_detect(site, "INT"), "3", site)), 
       aes(x = grass_nonnative*100, y = mature)) +
 ...
 geom_point(aes(color = site), size = 3, show.legend = TRUE) +
  ...
  labs(
    x = "Non-native grass (%)",
    y = "Mature shrub density (m^2)",
  #  title = "Mean stand height by Nonnative Grass percent cover",
     color = "Site") +
 
   stat_smooth(method = "lm", formula = y ~ poly(x, 2), se = TRUE, color = "red", linewidth = 1) +
    geom_smooth(aes(x = grass_nonnative * 100, y = mature), method = "lm", se = TRUE, linetype = "dashed", color = "gray40", show.legend = TRUE)
...
fig_den_immature <- ggplot(denspp_cover %>% 
 ...
 geom_point(aes(color = site), size = 3, show.legend = TRUE) +
  ...
  #  title = "Mean stand height by Nonnative Grass percent cover",
     ...
     stat_smooth(method = "lm", formula = y ~ poly(x, 2), se = TRUE, color = "red", linewidth = 1) +
    geom_smooth(aes(x = grass_nonnative * 100, y = immature), method = "lm", se = TRUE, linetype = "dashed", color = "gray40", show.legend = TRUE)
  ...
  labs(
    ...
    y = "Shrub seedling density (m^2)",
     color = "Site") +
      ...
      stat_smooth(method = "lm", formula = y ~ poly(x, 2), se = TRUE, color = "red", linewidth = 1) +
    geom_smooth(aes(x = grass_nonnative * 100, y = seedling), method = "lm", se = TRUE, linetype = "dashed", color = "gray40", show.legend = TRUE)
  ...
  labs(
    ...
    y = "Dead shrub density (m^2)",
     color = "Site") +
       ...
       stat_smooth(method = "lm", formula = y ~ poly(x, 2), se = TRUE, color = "red", linewidth = 1) +
    geom_smooth(aes(x = grass_nonnative * 100, y = dead), method = "lm", se = TRUE, linetype = "dashed", color = "gray40", show.legend = TRUE)
...
den_individual_shrubs %>% 
  dplyr::select(site_rep, species, seedling, cluster) %>% 
  filter(cluster == "NNG" &  seedling > 0)
den_individual_shrubs %>% 
  dplyr::select(cluster, site, rep, site_rep, immature, species) %>% 
  filter(cluster == "NNG" &  immature > 0)

den_individual_shrubs %>% 
  dplyr::select( site_rep, seedling, species, cluster) %>% # seedling
  filter(cluster == "ssG" &  seedling > 0)
den_individual_shrubs %>% 
  dplyr::select(cluster, site, rep, site_rep, immature, species) %>% # immature
  filter(cluster == "ssG" &  immature > 0)
...
denspp_cover %>% 
  left_join(groupsdash, by = "site_rep")  MONKEY HERE aug. 8, 2025
