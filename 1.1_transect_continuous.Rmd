...
percentcover <- abcover_omitground %>% 
  ...
  left_join(cover_bylfstatus, by = "join") %>% 
   dplyr::select(!c(join))
...
### percent cover by leaftype
```{r percent cover by leaftype}
cover_byleaftype <- abcover_omitground %>% 
  group_by(site_rep, lifeform_status_leaftype) %>% 
  dplyr::summarize(
    count_per_transect = n_distinct(site_rep_dist),
...
cover1 <- ggplot(data = cover_bylfstatus_wide, 
  ...
  labs(
    ...
    title = "Linear regression of non-native grass by native shrub percent cover"
    )
...
cover2 <- ggplot(data = cover_bylfstatus_wide, 
       aes(y = shrub_native*100, x = grass_nonnative*100, color = site)) + 
   geom_point(size = 3) +
  scale_color_manual(values = c("blue", "green", "maroon", "maroon", "maroon", "maroon", "maroon")) +
  geom_smooth(method = "lm", se = TRUE, color = "gray50", linetype = "solid")  +
  xlim(0, 100) +
  ylim(0, 100) +
  theme_bw() +
  labs(
    ...
    title = "Linear regression of native shrub by non-native grass percent cover"
    )
