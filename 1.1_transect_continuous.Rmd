...
nngxnatshr <- ggplot(data = cover_bylfstatus_wide %>% 
  ...
  mutate(cluster = case_when(
   ...
   scale_color_manual(values = c("orange", "grey", "hotpink", "blue", "green")) +
  # geom_smooth(method = "lm", se = TRUE, color = "gray50", linetype = "solid") + # se = confidence interval
   stat_smooth(method = "lm", formula = y ~ poly(x, 2), se = TRUE, color = "black", linewidth = 1) + 
  labs(
    #  title = "Linear regression of  total shrub cover by non-native grass cover",
...
model <- lm(shrub_native ~ grass_nonnative, data = cover_bylfstatus_wide) #linear regression (same as above)
model_seg <- segmented(model, seg.Z = ~grass_nonnative,
...
(AIC(model, model_poly, model_gam, model_seg))
...
plot_gam <- ggplot() +
  ...
  geom_point(data = cover_bylfstatus_wide,
             ...
             shape = 21, fill = "skyblue", color = "black", size = 3, alpha = 0.7) +
...
fig_thatch <- 
  ...
  geom_point(aes(color = site),
              ...
              position = position_jitter(width = 0.2, height = 0.1),
            show.legend = TRUE) +
   scale_color_manual(values = c("blue", "green", "maroon", "maroon", "maroon", "maroon", "maroon")) + # transects with no non-native thatch: INT5, 1_7
...
fig_nativelitter <- ggplot(data = groundcover_percent %>% 
                              mutate(site = ifelse(str_detect(site, "INT"), "3", site)), 
                           aes(x = grass_nonnative * 100, y = native_litter_wood*100)) +
  geom_point(aes(color = site),
              shape = 15,
