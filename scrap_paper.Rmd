 mutate(site = ifelse(str_detect(site, "INT"), "3", site)), 
  ...
  UNKNOWN forb 1 and 2 at Site 2_4:
  belt.spp_2019_load <- read.csv(here("data/Piru_Transect_Belt.xlsx", sheet == "belt_transects_raw")) %>% 
  dplyr::select(ch1_site, ch1_transect, ch1_species) %>% 
                             
  
  OMIT any variable that contains...                           
seedbankgerm_plantcluster %>%  
  filter(!str_detect(status, "Unknown"))
  
  OMIT exact specific variable...
  seedbankgerm_plantcluster %>%  
  filter(!status %in% "Unknown")
