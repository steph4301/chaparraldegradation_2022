...
abrich_totalrich <- abrich %>% # call tibble
  ...
  dplyr::summarize(total_species_richness = n_distinct(species)
               ...
               #     sqrt_trans = (max(abrich_totalrich$total_species_richness)-total_species_richness)/(sqrt(n_distinct(species))) # summarize the number of unique speices per site and rep 

# max(abrich_totalrich$total_species_richness)
# [1] 26
...
#rank based - quisqual wallis
#beta ray distribution - good for % cover distribution
```

```{r abrich total rich transform with taking square root}
# Reflect Data and use the appropriate transformation for right skew. Reflect every data point by subtracting it from the 
# maximum value. Add 1 to every data point to avoid having one or multiple 0 in your data.
...
ggplot(abrich_totalrich) + # no outliers in this dataset
  aes(x = "", y = total_species_richness) +
  geom_boxplot(fill = "#0c4c8a") +
  theme_minimal()
```

```{r abrich kruskal.test}
abrich_kw <- kruskal.test(total_species_richness ~ standtype, data = abrich_totalrich) 
abrich_kw

```

-   I can also calculate the **mean** and **standard errors** of the
    total species richness from each sampling location (belt and point).
...
#  column plot
abrich_totalrich_plot <- ggplot(abrich_totalrich_meanSD, aes(x = site, y = mean, fill = site)) +
...
-   
    ...
    2.  Percent cover by status by lifeform (fern, herb, shrub, grass)
        by stand
...
... and creating new columns for SITE_REP and SITE_REP_DISTANCE and
STAND_STATUS
...
... and then calculate means and SD by status (native, non-native) by
stand type.
