...
# install An Bui California color pallets
install.packages("devtools")
devtools::install_github("an-bui/calecopal")
```
```{r An Bui California color names}
names(cal_palettes)
```
...
shrubdensitylife_full <- read_csv("~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/deg_beltdensity_bylifestage.csv") #StandType, Site, rep, ScientificName, dead, immature, mature, resprout, seedling.

shrubdensityspp_full <- read_csv("~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/deg_beltdensity_byspecies.csv")
shrubdensityalive_full <- read_csv("~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/deg_beltdensity_seperatealive.csv")

# stand height 
standheight_full <- read_csv("~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/deg_maxheight.csv")
...
+ e. omit variable from data 
  filter(site_rep != "Intact4_4") %>% #omit Intact 4 from data
+ f. omit NAs from a column 
...
```{r abrich calculating total species richness mean and SD}
...
```{r abrich tukey posthoc, include=TRUE}
...
+ line graph
```{r abrich line graph abrich total species richness, include=TRUE}
#  line plot
...
... and creating new columns for SITE_REP and SITE_REP_DISTANCE and STAND_STATUS
...
abcover_full_v$stand_status <- paste(abcover_full_v$standtype, abcover_bystatus$status, sep="_") # combine stand and status label to one column
...
Now, I need to identify the unique species at each site_rep_dist to get rid of duplicate species recordings. (e.g., Present at one distance = 1/41 to get percent cover)
```{r ab percent cover unique species by site_rep_dist, inclue = TRUE}
...
head(abcover_bystatus) # prints to new tab
...
```{r ab percent cover aov by native status}
...
First, I need to count unique occurances of each native and non-native lifeform at each site_rep_dist so I can calculate % cover.
```{r percent cover of lifeform by status by site_rep, inclue = TRUE}
abcover_bylf_bystatus <- abcover_full_v_omitground %>% 
  group_by(standtype, site, rep, site_rep, lifeform, status) %>% 
  filter(!is.na(species)) %>% # omit blank cells in species 
  dplyr::summarize(count_per_dist = n_distinct(site_rep_dist), percentcover = n_distinct(site_rep_dist)/41)
# print(abcover_bystatus) # prints all rows of table
# head (abcover_bystatus) # prints first 6 rows of table
head(abcover_bylf_bystatus) # prints to new tab
```
Second I need to calculate means and SD by lifeform by status (native, non-native) by stand type. 
...
shrubdensitylife_transposded <- gather(shrubdensitylife_full, "status", "count", 5:9) 
head(shrubdensitylife_transposded)
```
... and create a new column for SITE_REP.
```{r shrub density site_rep column}
shrubdensity <- shrubdensitylife_transposded
