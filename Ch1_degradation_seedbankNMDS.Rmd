...
seedbank_load_v$stand <- factor(seedbank_load_v$stand, levels=c("intact", "enhanced", "degraded"), labels=c("Intact", "Matrix", "Degraded"))
```
...
seedbank_nmds_plot_color <- ggplot(nmds_df, aes(x = MDS1, y = MDS2, color = stand, shape = stand)) +
  ...
  theme_bw() +
  theme(legend.position = c(0.15, 0.15)) + # x, y left corner: 0,0
...
# START HERE - I NEED TO COMBINE DATA FRAMES TO RUN THE NEW PLOTS
...
seedbank_bysitecore_nmdsprep$standtype_ID <- 1:26
