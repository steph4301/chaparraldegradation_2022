...
unique(seedbank_load_v_omitpretreatment$treatment) # list unique treatments --> charate, control, oven, ovenchar
unique(seedbank_load_v_omitpretreatment$status) # list unique treatments --> native, nonnative
unique(seedbank_load_v_omitpretreatment$species) # list unique species --> 55 species
```
...
seedbank_bysitecore_nmdsprep_num <- seedbank_bysitecore_nmdsprep %>% 
  ...
  ungroup() %>% 
  select(!c(stand_core, stand, site)) %>% 
  replace(is.na(.), 0)
...
scores(seedbank_bysitecore_nmds) %>% 
  ...
  ggplot(aes(x=NMDS1, y=NMDS2, color = stand_core, shape = stand_core)) +
