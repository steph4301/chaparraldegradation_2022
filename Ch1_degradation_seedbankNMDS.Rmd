...
# total germination counts
```{r}
germination_m2_raw <- read_xlsx("processed/germination_m2.xlsx")
germination_m2 <- germination_m2_raw %>%
  mutate(site = fct_relevel(site, 
            "INT1", "INT2", "INT3","ENH1", "ENH2", "DEG1", "DEG2")) %>% 
  mutate(treatment = fct_relevel(treatment, 
                                 "control", "charate", "oven", "ovenchar"))
glimpse(germination_m2)
```

```{r}
germination_m2 %>%
   group_by(site) %>% 
  summarize(total_germ_m2 = sum(n_per_m2))
# site  total_native_germ_m2
# INT1	54,298.04			
# INT2	82,678.43			
# INT3	72,133.33			
# ENH1	128,114.29			
...
# DEG1	26,050.42			
# DEG2	15,546.22	
sort(unique(germination_m2$species_diversity))

germination_m2 %>%
  filter(status == "native") %>% 
  group_by(site) %>% 
  summarize(total_native_germ_m2 = sum(n_per_m2))
# site  total_native_germ_m2
# INT1	43,862.745			
# INT2	64,011.765			
# INT3	46,050.980			
...
germination_m2 %>%
  ...
  group_by(site) %>% 
  summarize(total_nonnative_germ_m2 = sum(n_per_m2))
# site  total_nonnative_germ_m2
# INT1	10,435.294			
# INT2	18,666.667			
# INT3	26,082.353			
# ENH1	107,885.714			
...
germination_m2 %>%
  ...
  group_by(site) %>% 
  summarize(total_forb_germ_m2 = sum(n_per_m2))
# site  total_forb_germ_m2
# INT1	25,647
# INT2	42,169			
# INT3	23,329			
# ENH1	23,314		
...
germination_m2 %>%
  ...
  group_by(site) %>% 
  summarize(total_shrub_germ_m2 = sum(n_per_m2))
# site  total_shrub_germ_m2
# INT1	11,710			
# INT2	26,988
# INT3	5,184		
# ENH1	10,743
...
germination_m2 %>%
  filter(lifeform == "shrub") %>% 
  filter()
  pull(species_diversity)


germination_m2 %>%
 filter(lifeform == "grass" & status == "native") %>% 
  group_by(site) %>% 
  summarize(total_nativegrass_germ_m2 = sum(n_per_m2))
# site  total_nativegrass_germ_m2
# INT1	13,063			
# INT2	2,682		
# INT3	20,976			
# ENH1	571			
# ENH2	686
```
# total native germination counts
```{r}
seedbank_load_v_omitpretreatment %>%  
  filter(status == "native") %>% 
  group_by(site) %>% 
  summarize(total_germination = sum(count))
# DEG1	34			
# DEG2	89			
# ENH1	139			
# ENH2	111			
...
```
# total non-native germination counts
```{r}
seedbank_load_v_omitpretreatment %>%  
  filter(status == "nonnative") %>% 
  group_by(site) %>% 
...
ggplot(seedbank_byfamily_proportion, aes(fill=family, y=proportion_germination, x=stand_core)) + 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
# stacked germination by family and status
...
#Stacked Germination by family with shrub -- Kit
```{r}
