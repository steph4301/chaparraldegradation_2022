...
germination_m2_raw_edit <- germination_m2_raw %>%
  ...
  dplyr::select(c(stand, site, soilcore, depth_name,fieldharvestedsoil_g, subsample_g,treatment, rep, species_diversity, genus_abundance, lifeform, status, lifescycle, family, stand_core, stand_core_depth, stand_core_depth_treat, count))
...
```{r explore fire following seedbanks SUM germination, include=TRUE}
seedbank_fire %>% 
  na.omit() %>%
...
```{r explore fire following seedbanks MEAN germination, include=TRUE}
seedbank_fire_totalemerg <- seedbank_fire %>% 
  na.omit() %>%
  group_by(site, stand_core) %>% 
  summarise(sum_germination_Nperm2 = sum(n_per_m2))
# site  stand_core sum_germination_Nperm2
#    <fct> <fct>                       <int>
#  1 INT1  INT1_1                       1831
#  2 INT1  INT1_3                        635
#  3 INT2  INT2_5                        266
...
seedbank_fire_totalemerg %>% 
  na.omit() %>%
  group_by(site) %>%  # group by site
  dplyr::summarise(mean_germination = mean(sum_germination_Nperm2), 
                    sd = sd(sum_germination_Nperm2),
                    N = length(sum_germination_Nperm2),
                    SE = (sd(sum_germination_Nperm2)) / sqrt(sum_germination_Nperm2)
                    ) 
