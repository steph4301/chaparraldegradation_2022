...
histogram(seedbank_fire_diversity_soil$fam_per_g)
...
boxplot(fam_per_g ~ site, 
        data = seedbank_fire_diversity_soil,
        main="Family Density by hillslope",
        xlab="Site", ylab="Family Density (#/g soil)")
```
...
ggqqplot(seedbank_fire_diversity_soil$fam_per_g) 
shapiro.test(seedbank_fire_diversity_soil$fam_per_g) 
# data:  seedbank_fire_diversity_soil$fam_per_g
# W = 0.90848, p-value = 0.1498
```
...
summary(aov(fam_per_g ~ site, data = seedbank_fire_diversity_soil))
#            Df    Sum Sq   Mean Sq F value Pr(>F)
# site         4 9.990e-06 2.498e-06   0.715  0.602
# Residuals    9 3.142e-05 3.491e-06       
```
...
# Residual standard error: 1.013 on 12 degrees of freedom
# Multiple R-squared:  0.003473,	Adjusted R-squared:  -0.07957 
# F-statistic: 0.04182 on 1 and 12 DF,  p-value: 0.8414
```

```{r family density by native shrub cover linear regression}
ggplot(data = seedbank_fire_diversity_soil, 
       aes(x = native_shrub*100, y = fam_per_g, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
#  geom_jitter() +
  ylim(0, 0.05)+
  xlim(0,100) +
  ...
  #                     ) + #rename standtypes labels
  labs(x = "Native shrub cover (%)", 
       y = "Density of fire following families (#/g soil)", 
       title = "Density of fire following species (#/g soil) by Native Shrub Cover") +
  theme_bw() +  
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2")))

seedbank_fire_famdensity_natshrub.lm = lm(fam_per_g ~ native_shrub, data=seedbank_fire_diversity_soil)
summary(seedbank_fire_famdensity_natshrub.lm)
```
...
ggplot(data = seedbank_fire_diversity, 
       aes(x = nonnative_grass*100, y = N_family, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
#  geom_jitter() +
  ylim(0, 5)+
  xlim(0,100) +
  ...
  labs(x = "Non-native grass cover (%)", 
       ...
       title = "Average number of fire following families by Non-native Grass Cover") +
  theme_bw() +  
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2")))

seedbank_fire_avgfamdiv_nngrass.lm = lm(N_family ~ nonnative_grass, data=seedbank_fire_diversity)
summary(seedbank_fire_avgfamdiv_nngrass.lm)
# Residual standard error: 0.7819 on 12 degrees of freedom
# Multiple R-squared:  0.08283,	Adjusted R-squared:  0.006404 
# F-statistic: 1.084 on 1 and 12 DF,  p-value: 0.3184
```

```{r family density by nonnative grass cover}

ggplot(data = seedbank_fire_diversity_soil, 
       aes(x = nonnative_grass*100, y = fam_per_g, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
#  geom_jitter() +
  ylim(0, 0.05)+
  xlim(0,100) +
  ...
  #                     ) + #rename standtypes labels
  labs(x = "Non-native grass cover (%)", 
       y = "Density of fire following species (#/g soil)", 
       title = "Density of fire following species (#/g soil) by Non-native Grass Cover") +
  theme_bw() +  
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2")))

seedbank_fire_famdensity_nngrass.lm = lm(fam_per_g ~ nonnative_grass, data=seedbank_fire_diversity_soil)
summary(seedbank_fire_famdensity_nngrass.lm)
# Residual standard error: 0.001803 on 12 degrees of freedom
# Multiple R-squared:  0.05818,	Adjusted R-squared:  -0.0203 
# F-statistic: 0.7413 on 1 and 12 DF,  p-value: 0.4061
```
