...
### - visualize data
```{r seed density with seperate soil depths visualization}
...
```{r seed density normality with seperate soil depths}
ggqqplot(seedbank_fire$n_per_g) 
...
```{r seed density by native shrub seperate soil depths}
...
```{r seed density by nn grass cover seperate soil depths}
...
```{r seed density by site stats seperate soil depths}
...
## b) COMBINING surface and below germination data
...
# site         4  2.867   0.7167    1.703     0.233 <-- no sig diff between sites
...
ggplot(data = seedbank_fire_totalemerg, # <-- not combining surface and below soil samples. 
       aes(x = native_shrub*100, y = sum_germ_g_log, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(-10, 0)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
...
```{r seed density by nonnative seperate soil depths}

# using NOT logged seed density data

ggplot(data = seedbank_fire_totalemerg, 
       aes(x = native_shrub*100, y = sum_germination_g, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(0, 0.5)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
  ...
  #                     ) + #rename standtypes labels
  labs(x = "Native shrub cover (%)", 
       y = "Seed density of Fire Followers (#/g))", 
       title = "Seed density of Fire Followers by Native Shrub Cover") +
  theme_bw() +  
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2")))

fire_nativeshrub.lm = lm(sum_germination_g ~ native_shrub, data=seedbank_fire_totalemerg)
summary(fire_nativeshrub.lm)
# Residual standard error: 0.009762 on 12 degrees of freedom
# Multiple R-squared:  0.1858,	Adjusted R-squared:  0.1179 
# F-statistic: 2.738 on 1 and 12 DF,  p-value: 0.1239 <-- not sig
...
ggplot(data = seedbank_fire_totalemerg, # <-- not combining surface and below soil samples. 
       aes(x = nonnative_grass*100, y = sum_germ_g_log, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(-10, 0)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
...
```{r seed density by nonnative grass cover combining soil depths not logged data}

# using NOT logged data

ggplot(data = seedbank_fire_totalemerg, # <-- not combining surface and below soil samples. 
       aes(x = nonnative_grass*100, y = sum_germination_g, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(0, 0.5)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
...
fire_seeddensity_nngrass.lm = lm(sum_germination_g ~ nonnative_grass, data=seedbank_fire_totalemerg)
summary(fire_seeddensity_nngrass.lm)
# Residual standard error: 0.009547 on 12 degrees of freedom
# Multiple R-squared:  0.2211,	Adjusted R-squared:  0.1562 
# F-statistic: 3.407 on 1 and 12 DF,  p-value: 0.08972 <-- not sig
```


```{r seed density by site combining soil depths}
# using not logged data

kruskal.test(sum_germination_g ~ site, data = seedbank_fire_totalemerg)
# data:  sum_germination_g by site
# Kruskal-Wallis chi-squared = 6.2081, df = 4, p-value = 0.1841 <-- not sig

pairwise.wilcox.test(seedbank_fire_totalemerg$sum_germination_g, seedbank_fire_totalemerg$site,
                 p.adjust.method = "BH")
```
...
seedbank_fire_diversity <- seedbank_fire %>% 
  group_by(stand_core, stand, site, soilcore, native_shrub, nonnative_grass) %>% 
...
boxplot(N_species ~ site, data=seedbank_fire_diversity, main="Species Diversity by hillslope",
...
```
### - absolute
```{r absolute spp and fam diversity by hillslope}
...
pairwise.wilcox.test(seedbank_fire_diversity$N_species, seedbank_fire_diversity$site,
                 p.adjust.method = "BH")
```
...
```{r absolute species diversity by native shrub cover}
# ABSOLUTE x native shrub cover

ggplot(data = seedbank_fire_absolute, 
       aes(x = native_shrub*100, y = total_spp_N, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(0, 10)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
  ...
  labs(x = "Native shrub cover (%)", 
       y = "Number of fire following species (#)", 
       title = "Number of unique fire following species by Native Shrub Cover") +
  theme_bw() +  
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2")))

seedbank_fire_absppdiv_natshrub.lm = lm(total_spp_N ~ native_shrub, data=seedbank_fire_absolute)
summary(seedbank_fire_absppdiv_natshrub.lm)
# Residual standard error: 1.15 on 3 degrees of freedom
# Multiple R-squared:  0.7692,	Adjusted R-squared:  0.6923 
# F-statistic: 9.999 on 1 and 3 DF,  p-value: 0.05079
...
```{r absolute species diversity by nn grass cover}
# ABSOLUTE x nn grass cover

ggplot(data = seedbank_fire_absolute, 
       aes(x = nonnative_grass*100, y = total_spp_N, color = site)) + #color = standtype
  geom_point(size = 3, alpha = 0.8) +
  ylim(0, 10)+
  xlim(0,100) +
  geom_smooth(method = "lm", se = FALSE, linetype = "solid", color = "black") +  # lm, loess, glm
  ...
  labs(x = "Non-native grass cover (%)", 
       y = "Number of fire following species (#)", 
       title = "Number of unique fire following species by non-native grass cover") +
  theme_bw() +  
  stat_poly_line() +
  stat_poly_eq(use_label(c("eq", "R2")))

seedbank_fire_absppdiv_nngrass.lm = lm(total_spp_N ~ nonnative_grass, data=seedbank_fire_absolute)
summary(seedbank_fire_absppdiv_nngrass.lm)
# Residual standard error: 1.425 on 3 degrees of freedom
# Multiple R-squared:  0.646,	Adjusted R-squared:  0.5279 
# F-statistic: 5.474 on 1 and 3 DF,  p-value: 0.1013 <-- not sig
