...
# Total germ of firefollowing
```{r total germination, include=TRUE}
...
# site    total_germination   total_species   average_germ_pergsoil
# INT1	    10    	          3                 0.005		
# INT2    	7	                4                 0.005		
# INT3    	11	              3                 0.004		
# ENH1	    9	                4                 0.004		
# ENH2    	11	              8                 0.003	
```
```{r total germination visualization}
boxplot(n_per_g ~ site, data=seedbank_fire, main="Seeds/g by site",
   xlab="Site", ylab="Fire Following seeds per gram")
histogram(seedbank_fire$n_per_g)
length(seedbank_fire$n_per_g) #44 
...
# W = 0.78523, p-value = 1.414e-06 <-- both aren't great :/

# To determine: do ENH and INT hillslopes have sig different fire following seed densities? or are they the same?

```
### differences seed density by site
```{r seed density by site normality}
kruskal.test(n_per_g ~ site, data = seedbank_fire)
# data:  n_per_g by site
# Kruskal-Wallis chi-squared = 6.4651, df = 4, p-value =
# 0.167
