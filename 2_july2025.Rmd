...
# NOTE: extracted df is loaded in 2.1_transect_compile_environdata.Rmd (july 22, 2025 - SML)

extracted_load <- read.csv("~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/chaparraldegradation_2022/chaparraldegradation_2022/Extracted_Elevation_Slope_Aspect_Hillshade_20m_20241014_8pm.csv", header = TRUE, na.strings=c("","NA")) 
...
######### calculate means of environmental data
means <- extracted %>%
...
boxdf <-  extracted %>% 
  dplyr::select(-group_15, -group_3, -group_10, -group_6, -group_4, -group_2) %>% 
  # rownames_to_column(var = "rowname") %>% 
   # full_join(my_groups, by = "rowname") %>% 
  mutate(fire_history = case_when(
  fire_history == "2007" ~ 1,
  fire_history == "2003_2007" ~ 2
))
  rename(environ_cluster = sub_grp) %>% 
     mutate(environ_cluster = as.character(environ_cluster)) %>% 
   mutate(plant_cluster = as.character(plant_cluster))
