...
```{r percentcoverprep, echo=FALSE, eval=F}

# percent cover - belt transects, point intercept transects
abcover_full <- read.csv("data/deg_percentcover_byspecies_bylifeform_2024.csv", header = TRUE, na.strings=c("","NA")) %>%  
  dplyr::select(-species_code, -notes, -hillslope, -standtype) %>% 
  na.omit()
...
abcover_full_v$site_transect <- paste(abcover_full_v$site, abcover_full_v$transect, sep="_") 

# combine site_rep and distance to one column
abcover_full_v$site_transect_dist <- paste(abcover_full_v$site_transect, abcover_full_v$distance, sep="_") 
...
abcover_full_v %>%  
 # na.omit() %>%  
  ...
  summarize(hits = length(unique(distance)))

# omit "ground" as a status since interested in Native and Non-native species 
abcover_full_v_omitground <- abcover_full_v %>%
  filter(status %in% c("native", "nonnative", "Native", "Non-native"))
...
```{r FIGUREStotalvegcover, include=TRUE}
...
# end jitter code
```
```{r totalvegnormality, include=TRUE}
...
abcover_vegcover_long_vegonly$site_rep <- paste(abcover_vegcover_long_vegonly$site, abcover_vegcover_long_vegonly$transect, sep="_") # combine site and rep form to one column
```

#### categorical by stand type
```{r totalvegstandtypeCAT, include=TRUE}
...
```
#### categorical by hillslope
```{r totalveghillslopeCAT, include=TRUE}

# code from --> http://www.sthda.com/english/wiki/kruskal-wallis-test-in-r

kruskal.test(percentcover ~ site, data = abcover_vegcover_long_vegonly)
# 	Kruskal-Wallis rank sum test
# 
# data:  percentcover by site
# Kruskal-Wallis chi-squared = 6.7264, df = 6, p-value = 0.3469

pairwise.wilcox.test(abcover_vegcover_long_vegonly$percentcover, abcover_vegcover_long_vegonly$site,
                 p.adjust.method = "BH")
# data:  abcover_vegcover_long_vegonly$percentcover and abcover_vegcover_long_vegonly$site 
# 
...
```
```{r meancoverbysite, include=TRUE}
...
abcover_bylf_bystatus <- abcover_full_v_omitground %>% 
  ...
  ungroup() %>% 
  unite("lifeform_total", 2:3, sep = "_")
...
```{r percent cover stacked by lifeform}


lifeformstatus_stacked <- ggplot(lifeformleaftype, 
       aes(x = site_rep, y = percentcover*100, 
           fill = lifeform_status)
       ) +
  geom_col(position = "stack") +
  labs(title = "Vegetation cover by lifeform", 
       x = "Site", 
       y = "Percent Cover (%)") +
  scale_fill_manual(values = c("gold", "orange", "brown", "limegreen", "deeppink","purple", "darkgreen"),
                     name = "Lifeform - Status",
                     labels = c("grass_native" = "native grass", 
                                "herb_native" = "native forb", 
                                "nonsclero_native" = "native non-sclerophyllous_native shrub",
                                 "sclerophyllous_native" = "native sclerophyllous_native shrub",
                                "grass_nonnative" = "non-native grass", 
                                
                                "herb_nonnative" = "non-native forb"
                               )) + #rename standtypes labels
  ylim(0,250) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
lifeformstatus_stacked

ggsave("Ch1_Degradation_figures/lifeformstatus_stacked.png", lifeformstatus_stacked, width = 8, height = 6)
```
...
##### - by hillslope
```{r leaf type stats - SCL - hillslope}
...
```
```{r SCL percent cover by hillslope figure}
ggplot(shrubspp_leaftype_SCLcover, 
       aes(x = site, y = sum_leaftype_cover)) +
  geom_boxplot() +
  theme(axis.text.x.bottom = element_text(angle = 90)) +
  labs(title = "Sclerophyllous shrub cover by hillslope", 
       x = "Hillslope",
       y = "Percent Cover (%)")
```

##### - by stand type
```{r leaf type stats - SCL - stand type}
SCLcover_bysite <- shrubspp_leaftype_SCLcover %>%  
  group_by(standtype, site) %>% 
  summarise(meanSITE_SCLcover = mean(sum_leaftype_cover))

histogram(SCLcover_bysite$meanSITE_SCLcover)

shapiro.test(SCLcover_bysite$meanSITE_SCLcover)
# Shapiro-Wilk normality test
# 
# data:  SCLcover_bysite$meanSITE_SCLcover
# W = 0.78369, p-value = 0.02815 <-- not normally distributed

### by stand type
kruskal.test(meanSITE_SCLcover ~ standtype, data = SCLcover_bysite)
# Kruskal-Wallis rank sum test
# 
# data:  meanSITE_SCLcover by standtype
# Kruskal-Wallis chi-squared = 5.0769, df = 2, p-value = 0.07899

pairwise.wilcox.test(SCLcover_bysite$meanSITE_SCLcover, SCLcover_bysite$standtype,
                 p.adjust.method = "BH")
#     intact matrix
# matrix   0.30   -     
# degraded 0.30   0.62  
...
```{r SLC percent cover by standtype figure}
ggplot(SCLcover_bysite, 
       aes(x = standtype, y = meanSITE_SCLcover)) +
  geom_boxplot() +
  labs(title = "Sclerophyllous shrub cover by standtype", 
       x = "Standtype",
       y = "Percent Cover (%)")
```
...
```{r SCLlmer}
SCLlmer <- lmer(sum_leaftype_cover ~ standtype + (1|standtype/site), shrubspp_leaftype_SCLcover)
summary(SCLlmer)
# Fixed effects:
#                   Estimate Std. Error      df t value Pr(>|t|)  
# (Intercept)         52.846     12.182   4.325   4.338   0.0103 *
# standtypematrix    -51.220     19.117   4.199  -2.679   0.0525 .
# standtypedegraded  -52.846     19.117   4.199  -2.764   0.0480 *
# ---
...
# site         6   5812   968.6   3.172 0.0673 .<-- no sig diff
...
# ENH1-INT1  58.536585  -18.49934 135.57251 0.1646056
# ENH2-INT1  26.829268  -50.20665 103.86519 0.8220705
# DEG1-INT1  19.512195  -57.52373  96.54812 0.9483739
# DEG2-INT1   8.130081  -68.90584  85.16600 0.9994274
# INT3-INT2 -34.146341 -128.49569  60.20301 0.7979319
# ENH1-INT2   2.439024  -74.59690  79.47495 0.9999995
# ENH2-INT2 -29.268293 -106.30422  47.76763 0.7644413
# DEG1-INT2 -36.585366 -113.62129  40.45056 0.5737545
# DEG2-INT2 -47.967480 -125.00340  29.06844 0.3149369
# ENH1-INT3  36.585366  -40.45056 113.62129 0.5737545
# ENH2-INT3   4.878049  -72.15787  81.91397 0.9999701
# DEG1-INT3  -2.439024  -79.47495  74.59690 0.9999995
# DEG2-INT3 -13.821138  -90.85706  63.21478 0.9899234
# ENH2-ENH1 -31.707317  -86.17994  22.76531 0.3764956
# DEG1-ENH1 -39.024390  -93.49701  15.44823 0.2032746
# DEG2-ENH1 -50.406504 -104.87913   4.06612 0.0724026
# DEG1-ENH2  -7.317073  -61.78970  47.15555 0.9978115
# DEG2-ENH2 -18.699187  -73.17181  35.77344 0.8306002
# DEG2-DEG1 -11.382114  -65.85474  43.09051 0.9786122
```

```{r NONSCL percent cover by hillslope}
ggplot(shrubspp_leaftype_SCLcover, 
       aes(x = site, y = sum_leaftype_cover)) +
  geom_boxplot() +
  theme(axis.text.x.bottom = element_text(angle = 90)) +
  labs(title = "Non-sclerophyllous shrub cover by hillslope", 
       x = "Hillslope",
       y = "Percent Cover (%)")
```
...
```{r NONSCL percent cover by stand type stats}

NONSCLcover_byhillslope <- shrubspp_leaftype_NONSCLcover %>% 
  group_by(standtype, site) %>% 
  summarise(meansite_NONSCLcover = mean(sum_leaftype_cover))

histogram(NONSCLcover_byhillslope$meansite_NONSCLcover)

# NONSCL by hillslope
shapiro.test(NONSCLcover_byhillslope$meansite_NONSCLcover)
# Shapiro-Wilk normality test
# 
# data:  NONSCLcover_byhillslope$meansite_NONSCLcover
# W = 0.90062, p-value = 0.3347 <-- normally distributed

summary(aov(meansite_NONSCLcover ~ standtype, data = NONSCLcover_byhillslope))
#    Df Sum Sq Mean Sq F value Pr(>F)
# standtype    2  841.6   420.8   0.777  0.519 <-- no sig diff
# Residuals    4 2165.7   541.4   

TukeyHSD((aov(meansite_NONSCLcover ~ standtype, data = NONSCLcover_byhillslope)))
# $standtype
#                      diff        lwr      upr     p adj
# matrix-intact    16.66667  -59.03679 92.37012 0.7312194
# degraded-intact -12.19512  -87.89857 63.50833 0.8404796
# degraded-matrix -28.86179 -111.79077 54.06719 0.4940505
```

```{r NONSLC percent cover by standtype figure}
ggplot(NONSCLcover_byhillslope, 
       aes(x = standtype, y = meansite_NONSCLcover)) +
  geom_boxplot() +
  labs(title = "Non-sclerophyllous shrub cover by standtype", 
       x = "Standtype",
       y = "Percent Cover (%)")
```
...
#### - NONSCL by LMER
```{r}
NONSCLlmer <- lmer(sum_leaftype_cover ~ standtype + (1|site), 
...
shrubcover <- abcover_bylf_bystatus %>%  filter(lifeform == "shrub")
shrubcover$standtype <- factor(shrubcover$standtype, 
                               levels = c("degraded", "matrix", "intact") # set degraded as intercept
                               )
shrublmer <- lmer(percentcover ~ standtype + (1|site), 
                data = shrubcover)

summary(shrublmer)
tab_model(shrublmer, transform = NULL)
```
...
##### - nngrass cover by hillslope

```{r nonnative grass by hillslope stats}
...
# 
# data:  percentcover by site
# Kruskal-Wallis chi-squared = 13.099, df = 6, p-value = 0.0415
...
# INT3 0.62 0.62 0.62 0.62 1.00 1.00 <-- no sig differences
```

```{r nngrass cover by hillslope figure}
ggplot(abcover_nngrass, 
       aes(x = site, y = percentcover)) +
  geom_boxplot() +
  theme(axis.text.x.bottom = element_text(angle = 90)) +
  labs(title = "Non-native grass cover by hillslope", 
       x = "Hillslope",
       y = "Percent Cover (%)")
```

##### - nngrass cover by stand type

```{r nngrass cover by stand type stats}

abcover_nngrass_byhillslope <- abcover_nngrass %>% 
  group_by(standtype, site) %>% 
  summarise(mean_percentcover = mean(percentcover))

kruskal.test(mean_percentcover ~ standtype, data = abcover_nngrass_byhillslope)
# Kruskal-Wallis rank sum test

# data:  mean_percentcover by standtype
# Kruskal-Wallis chi-squared = 5.3571, df = 2, p-value = 0.06866 <-- normally distributed

summary(aov(mean_percentcover ~ standtype, data = abcover_nngrass_byhillslope))
#  Df Sum Sq Mean Sq F value  Pr(>F)   
# standtype    2 0.8862  0.4431   44.77 0.00183 **
# Residuals    4 0.0396  0.0099                   
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1  

TukeyHSD((aov(mean_percentcover ~ standtype, data = abcover_nngrass_byhillslope)))
# $standtype
#                      diff        lwr       upr     p adj
# matrix-intact   0.5894309  0.2657448 0.9131170 0.0064030 <-- ENH and INT
# degraded-intact 0.8089431  0.4852570 1.1326292 0.0019497 <-- DEG and INT
# degraded-matrix 0.2195122 -0.1350681 0.5740925 0.1836405
```

```{r nngrass percent cover by standtype figure}
ggplot(abcover_nngrass_byhillslope, 
       aes(x = standtype, y = mean_percentcover)) +
  geom_boxplot() +
  labs(title = "Non-native grass cover by standtype", 
       x = "Standtype",
       y = "Percent Cover (%)")
```
...
abcover_nativecover_nngrasscover <- left_join(abcover_nativecover, 
                                              cover_statuslf_matrix_wide1, by = "site_rep")
...
# CANOPY COVER - Bird's eye view

```{r canopy cover data prep}
  
whatsincanopy <- lifeformleaftype %>% 
  group_by(site_rep_dist) %>%
  slice_max(order_by = vertical, n = 1) %>% # no ferns 
   mutate(lifeform_status = paste(lifeform, status, sep = "_")) %>% 
  group_by(standtype, site, rep, site_rep, lifeform, status, lifeform_status) %>% 
  dplyr::summarize(
    count_per_transect = n_distinct(site_rep_dist), 
  ...
  mutate(site_rep = fct_relevel(site_rep, "INT1_1", "INT2_2", "INT3_3", "ENH1_1" ,    "ENH1_2"   ,  "ENH1_3" ,    "ENH2_4" ,    "ENH2_5"    ,"ENH2_6", "DEG1_1",     "DEG1_2"  ,   "DEG1_3"   ,  "DEG2_4" ,    "DEG2_5"   ,  "DEG2_6")) %>% 
  mutate(lifeform_status = fct_relevel(lifeform_status, "grass_nonnative", "herb_nonnative",
                                       "grass_native", "herb_native", "nonsclero_native", "sclerophyllous_native")
  )
  
```

```{r canopy cover figure}


  
canopycover_figure <- ggplot(whatsincanopy, 
       aes(x = site_rep, y = percentcover*100, 
           fill = lifeform_status)
       ) +
  geom_col(position = "stack") +
  labs(title = "Canopy Cover", 
       x = "Site", 
       y = "Percent Cover (%)") +
  scale_fill_manual(values = c("gold", "orange", "limegreen", "deeppink","purple", "darkgreen"),
                     name = "Lifeform - Status",
                     labels = c("grass_native" = "native grass", 
                                "herb_native" = "native forb", 
                                "nonsclero_native" = "native non-sclerophyllous_native shrub",
...
# 
# ggplot(whatsincanopy, 
#        aes(x = site_rep, y = percentcover*100, 
#            fill = lifeform_status)
#        ) +
#   geom_col(aes(fill = factor(lifeform_status, 
#                              levels = c("grass_native", "herb_native", "shrub_native",
#                                         "grass_nonnative", "herb_nonnative"))), 
#            position = "stack") +
#   geom_col_pattern(data = filter(whatsincanopy, 
#                                  lifeform_status %in% c("herb_nonnative", "grass_nonnative")), 
#                    aes(fill = lifeform_status),
#                    pattern = "stripe", pattern_angle = 45, pattern_density = 0.1, position = "stack")  +
#   labs(title = "Canopy Cover", 
#        x = "Site", 
#        y = "Percent Cover (%)") +
#   scale_fill_manual(values = c(shrub_native = "darkgreen", 
#                                herb_native = "blue",
#                                grass_native = "orange", 
#                                 herb_nonnative = "blue", 
#                                grass_nonnative = "orange"
#                                 ),
#                      name = "Lifeform - Status",
#                      labels = c("grass_native" = "native grass", 
#                                 "grass_nonnative" = "non-native grass", 
#                                 "herb_native" = "native forb", 
#                                 "herb_nonnative" = "non-native forb", 
#                                 "shrub_native" = "native shrub")) + #rename standtypes labels
#   ylim(0, 100) +
#   theme_bw() +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
#   
...
canopycover_figure
ggsave("Ch1_Degradation_figures/canopycover_figure.png", canopycover_figure, width = 8, height = 6)

combined_caonpyplots <- grid.arrange(canopycover_figure + theme(
                                            legend.position = c(0.4, 0.825),
                                            legend.key.size = unit(0.2, "cm")
                                             ),
                                     lifeformstatus_stacked + theme(
                                       legend.position = "none"), 
                                     ncol = 2, 
                                widths = c(4, 4),  # Adjust the width of each plot
                                heights = c(6)) # Adjust the height of each plot
ggsave("Ch1_Degradation_figures/combined_caonpyplots.png", combined_caonpyplots, width = 8, height = 6)

```

```{r}
view(abcover_raw %>%
       filter(site == "ENH1" & status == "nonnative" & lifeform == "herb"))
```
...
# write.csv(abcover_byfamily_percent,"processed/abcover_byfamily_percent.csv")
...
# Kruskal-Wallis chi-squared = 98.456, df = 6, p-value
# < 2.2e-16
...
# join native and non-native cover to stand height
standheight_withcover <- standheight_full %>% 
  left_join(nativecover, by = "site_rep") %>% 
  group_by(site, site_rep, native_shrub, nonnative_grass) %>% 
  summarize(mean_height = mean(height_cm))

height_native <- ggplot(standheight_withcover,
...
summary(lm(mean_height ~ native_shrub, data = standheight_withcover))
...
summary(lm(mean_height ~ nonnative_grass, data = standheight_withcover))
...
```{r left join percent cover to alive shrub density}

nativecover_fordensity <- nativecover %>% #created for stand height and cov
  mutate(site_rep = case_when(
    site_rep == "Intact_1_1" ~ "Intact1_1", # old name ~ new name
       site_rep == "Intact_2_2" ~ "Intact2_2",
       site_rep == "Intact_3_3" ~ "Intact3_3",
  ...
  dplyr::select(site_rep, native_shrub, nonnative_grass)

view(nativecover_fordensity)

```

```{r left join native and non-native cover to density data}

shrubdensity_sum_bysiterep_cover <- shrubdensity_sum_bysiterep %>%
 left_join(nativecover_fordensity, by = "site_rep") 
view(shrubdensity_sum_bysiterep_cover)
```

### figure - native shrub cover
```{r sandbox}
...
```{r linear regression - density x nat cover}
alivedensity_shrubcover_linear <- ggplot(shrubdensity_sum_bysiterep_cover, 
       aes(x = native_shrub*100, y = aliveshrubdensity_1m2, olor = Hillslope)) + 
  geom_point(size = 3, alpha = 0.8) +
...
shrubdensity_sum_bysiterep_regression <- ggplot(shrubdensity_sum_bysiterep_cover, 
