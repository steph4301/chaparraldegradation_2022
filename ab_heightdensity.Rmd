...
```{r}
ggplot(standheight, aes(x = as.integer(reading)/2, y = height_cm/100, group = site_rep, color = standtype)) +
...
shrubdensity_2019 <- gather(shrubdensitylife_load, "lifestage", "density_80m2", 4:8) %>% 
  mutate(year = 2019)

#view(shrubdensity_2019) # look at new column site_rep, last column
# check for missing data

#google drive data
shrubdensity2024_loadfromgoogle <- "https://docs.google.com/spreadsheets/d/1FK2YxFNLESzUOiZXI4AIm9gUVStaOLQqCRljIOMnwoM/edit"

shrubdensity_2024 <- read_sheet(shrubdensity2024_loadfromgoogle, sheet = "shrub_density") %>%
  mutate(
    site = as.character(site),
    transect = as.character(transect),
    species = as.character(species),
    density_80m2 = as.integer(density_80m2)
  ) %>%
  dplyr::select(-date, -name, -hillslope, -standtype, -notes) %>% 
  mutate(year = 2024)

#view(shrubdensity_2024)

shrubdensity <-  full_join(shrubdensity_2019, shrubdensity_2024, by = c("site", "transect", "species", "lifestage", "density_80m2", "year")) %>% # Perform the join using all common columns
  mutate(site_transect = paste(site, transect, sep = "_")) %>%  # combine site and transect to one column
   mutate(site_transect_species = paste(site_transect, species, sep = "_"))

shrubdensity %>% 
  group_by(site, transect) %>% 
  summarize(length_count = length(site_transect))
...
sort(unique(shrubdensity$site_transect))
sort(unique(shrubdensity$species))
sort(unique(shrubdensity$site_transect_species))

#write.csv(shrubdensity,"processed/shrubdensity.csv") 
#last saved on May 16, 2024 with Site 1: transect 1-10, Site 2: transect 1-18
...
# TOTAL shrub density
```{r shrub dentsity 100m2, include=TRUE}

tot_shrubdensity <- shrubdensity %>% # call tibble
  group_by(site, transect) %>%  # lump immature, mature, resprout shrubs and seedlings
  summarize(shrubdensity_80m2 = sum(density_80m2), 
            shrubdensity_100m2 = round(((shrubdensity_80m2/80)*100), digits = 0))  # 4m x 20 m belt transect
view(tot_shrubdensity) 
```

## FIGURE 3 - shrub density by species, lifestage
```{r figure 3}
spp_lifeform <- shrubdensity %>% # call tibble
   mutate(site = case_when(
  ...
  group_by(standtype, Hillslope, site_rep, species, status) %>%  # lump immature, mature, resprout shrubs and seedlings
  summarize(shrubdensity_80m2 = sum(count), 
            shrubdensity_100m2 = round(((shrubdensity_80m2/80)*100), digits = 0)) %>%  # 4m x 20 m belt transect
   dplyr::select(!c("shrubdensity_80m2")) %>% 
  group_by(Hillslope, species, status) %>% 
  summarize(mean_shrubdensity_100m2 = round(mean(shrubdensity_100m2), digits = 1),
            n = length((shrubdensity_100m2)),
            sd = sd(shrubdensity_100m2),
            se = round((sd(shrubdensity_100m2))/sqrt(length((shrubdensity_100m2))), digits = 1)
            )

view(spp_lifeform)
write.csv(spp_lifeform,"processed/spp_lifeform.csv")
```

- visualize
```{r total density visualize}
hist(tot_shrubdensity$shrubdensity_100m2)

# by hillslope
boxplot(shrubdensity_100m2 ~ Hillslope, data = tot_shrubdensity, col = "red")
ggplot(tot_shrubdensity, aes(x = Hillslope, y = shrubdensity_100m2, color = site_rep)) +
  geom_point() +
  labs(x = "Hillslope", y = "Total Shrub Density (per 100m^2)",
       title = "Scatter Plot of Hillslope vs Total Shrub Density")

# by standtype
boxplot(shrubdensity_100m2 ~ standtype, data = tot_shrubdensity, col = "red")
ggplot(tot_shrubdensity, aes(x = standtype, y = shrubdensity_100m2)) +
  geom_point(aes(color = Hillslope, shape = Hillslope), size = 3, alpha = 0.8) +
  labs(x = "Standtype", y = "Total Shrub Density (per 100m^2)",
       title = "Scatter Plot of Total Shrub Density by Standtype")

```


```{r total shrub density stats}
shapiro.test(tot_shrubdensity$shrubdensity_100m2)
# Shapiro-Wilk normality test
# 
# data:  tot_shrubdensity$shrubdensity_100m2
# W = 0.47102, p-value = 2.071e-06 <-- not normally distributed
```

 - KW total shrub density 
```{r total density by site stats, include = TRUE}
# by Hillslope
kruskal.test(shrubdensity_100m2 ~ Hillslope, data = tot_shrubdensity)
# Kruskal-Wallis rank sum test
# 
# data:  shrubdensity_100m2 by Hillslope
# Kruskal-Wallis chi-squared = 10.577, df = 6, p-value = 0.1024 <-- no diff by hillslope
pairwise.wilcox.test(tot_shrubdensity$shrubdensity_100m2, tot_shrubdensity$Hillslope,
                 p.adjust.method = "BH")

# by standtype
kruskal.test(shrubdensity_100m2 ~ standtype, data = tot_shrubdensity)
# Kruskal-Wallis rank sum test
# 
# data:  shrubdensity_100m2 by standtype
# Kruskal-Wallis chi-squared = 9.0328, df = 2, p-value = 0.01093 <-- sig diff by standtype
pairwise.wilcox.test(tot_shrubdensity$shrubdensity_100m2, tot_shrubdensity$standtype,
                 p.adjust.method = "BH")
# Pairwise comparisons using Wilcoxon rank sum exact test 
# 
# data:  tot_shrubdensity$shrubdensity_100m2 and tot_shrubdensity$standtype 
# 
#          intact matrix
# matrix   0.548  -     
# degraded 0.041  0.039  <-- DEG is diff from INT and ENH

tot_shrubdensity_mean <-tot_shrubdensity %>%
  group_by(standtype) %>%
  summarize(mean_totshrubdensity_100m2 = round(mean(shrubdensity_100m2), digits = 0),
            n = length((shrubdensity_100m2)),
            sd = round(sd(shrubdensity_100m2), digits = 1),
            se = round(((sd(shrubdensity_100m2))/sqrt(length((shrubdensity_100m2)))), digits = 1)
            )
...
```{r total shrub dentsity 100m2 - figures - tables, include=TRUE}

total_shrubdensity_column <- ggplot(tot_shrubdensity_mean, aes(x = standtype, y = mean_totshrubdensity_100m2, fill = standtype)) +
  geom_col(position = "dodge", width = 0.8) +
  geom_errorbar(
    aes(ymin=mean_totshrubdensity_100m2-se, ymax=mean_totshrubdensity_100m2+se), 
                width=.2, position=position_dodge(.9)) +
  labs(
    title = "Total shrub density by stand type", 
    x = "Stand Type", 
    y = "Average total shrub density (#/100m2)") + # title, axes titles
  ...
  theme_bw()
total_shrubdensity_column
```
```{r table of shrubs}
# table: standtype, species, lifestage
tot_shrubdensity_table <- shrubdensity %>%
  # mutate(status = case_when(status == "resprout" ~ "immature", 
  #                           TRUE ~ status)) %>% 
  mutate(status = fct_relevel(status, "mature", "immature", "resprout", "seedling", "dead")) %>% 
  group_by(standtype, site_rep, status) %>%
   rename(lifestage = status) %>% 
  summarize(density_100m2 = (((sum(count))/80)*100) 
            ) %>%
  group_by(standtype, lifestage) %>% 
  summarize(mean_density_100m2 = round(mean(density_100m2),digits = 1),
            n = length((density_100m2)),
            sd = round(sd(density_100m2), digits = 1),
            se = round(((sd(density_100m2))/sqrt(length((density_100m2)))), digits = 1)) %>% 
  print(tot_shrubdensity_table) 

#write.csv(tot_shrubdensity_table,"processed/tot_shrubdensity_table.csv")


```

 - dead shrubs
```{r dead shrubs at MX1}
deadshrubs <- shrubdensity %>% # call tibble
  filter(status == "dead") %>% #select dead shrubs
   mutate(site = case_when(
    site == "Intact1" ~ "SD1", # old name ~ new name
      site == "Intact2" ~ "SD2",
      site == "Intact3" ~ "SD3",
  ...
  rename(Hillslope = site) %>% # old name = new name
  mutate(Hillslope = fct_relevel(Hillslope, "SD1", "SD2", "SD3", "MX1", "MX2", "GD1", "GD2")) %>% 
  group_by(standtype, Hillslope, species) %>%  # lump immature, mature, resprout shrubs and seedlings
  summarize(deadshrubdensity_80m2 = sum(count), 
             deadshrubdensity_100m2 = round(((deadshrubdensity_80m2/80)*100), digits = 0))  %>% # 4m x 20 m belt transect
  filter(deadshrubdensity_100m2 > 0)
view(deadshrubs) 
...
shrubdensity_sum_bysiterep <- shrubdensity %>% # call tibble
  ...
  summarize(aliveshrubdensity_80m2 = sum(count), 
            aliveshrubdensity_100m2 = round(((aliveshrubdensity_80m2/80)*100), digits = 0))  # 4m x 20 m belt transect
...
shrubdensity_table_spread %>% 
  ...
  summarize(sum_mature = sum(mature), 
            ...
            sum_resprout = sum(resprout),
            sum_seedling = sum(seedling), 
            sum_dead = sum(dead))
#standtype  sum_mature  sum_immature    sum_resprout    sum_seedling        sum_dead
#intact	    101	        44	              1               7	                  22
#matrix	    297	        67	              3               5	                  256
#degraded	  72	        15	             1                  3	                  0

# INT - proportion mature shrubs
round(101/(101+44+1+7)*100, digits = 2)
# [1] 66.01
```
