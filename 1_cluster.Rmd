...
plot(hcluster.abs.cover, labels = cluster.subgrp.df$sub_grp, main = "Hierarchical Clustering Dendrogram")
rect.hclust(hcluster.abs.cover, k = 5)
...
pv <- pval$vectors$pvals %>% # extract p-values for each species
  as.data.frame() %>% 
...
ggplot(mds.df,
  ...
  geom_segment(data = spp, 
               ...
               col = "black") +
  geom_text(data = spp, aes(label = species),  
