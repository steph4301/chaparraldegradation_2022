...
Load data
```{r load point intercept transect data from csv, include=TRUE, eval = FALSE}


# july 2015 - moved this chunck to 0_dataprep.Rmd
...
abcover_fulljoin <- full_join(transect2019, transect2024, 
                              by = intersect(names(transect2019),
                                             names(transect2024)))
...
########### this is old code that: pulled cluster (aka. sub_group numbers from the environmental data)
########### and then created two versions (1) with a dot between site and transect number <-- "groups" 
########### and (2) with a underscore line between site and transect number <-- "groups_dash
...
length(unique(specieslist$species))
```
...
### by CLUSTER
```{r}
# prep df to left join cluster NAMES with transect data percent cover and sppchar by site_rep
cluster_prep <- cluster.subgrp.df %>% 
  dplyr::select(site_rep, sub_grp, sub_grp_name, combined_labels)    


# prepare % cover data from data_pc <-- from 0_dataprep.Rmd
# this df does not include dead species, groundcover, or unknown 1 forb
...
cover <- data_pc %>% 
  ...
  # left join percent cover data with species characteristics
  left_join(sppchar, by = "species") %>% 
  
  left_join(cluster_prep, by = "site_rep") %>% 
  
  # 52 species * 40 transects = 2080 <--  without omitting rows with 0%, there are 2080 data entries
  # omit percent cover values that are = 0%
  filter(percentcover > 0) %>% 
  mutate(lifeform_lifecycle_status = paste(lifeform, lifecycle, status, sep = "_"))
...
# goals - create a df with cover by lifeform by lifecycle
cover_cluster <- cover %>% 
  group_by(sub_group_name, lifeform, lifecycle, status, lifeform_lifecycle, lifeform_lifecycle_status) %>% 
...
```{r, eval = FALSE}
# calculate % cover from the raw transect data (df: abcover_omitground)
cover_bylifecycle <- abcover_omitground %>%  # <- transect data has vertical and distance still
...
# prepare cluster names to left join with raw transect data
groups <- cluster.subgrp.df %>% # <- from 1_cluster.Rmd
  
  dplyr::select(rowname, site_rep, sub_grp, sub_grp_name, combined_labels) %>% 
    separate(site_rep, into = c("site", "rep"), remove = FALSE) %>% 
...
fig_forbgrass <- ggplot(cover_bylifecycle_bygroup %>%
  ...
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 0, size = 10, hjust = 1),  # Rotate x-axis labels if needed
...
densitymeans_bycluster <- density_cover_cluster %>% # without shrub species
  ...
  dplyr::summarise(
    median_density = median(sum_density_m2, na.rm = TRUE), 
    n = n_distinct(site_rep),
