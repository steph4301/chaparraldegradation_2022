...
seedbank_cluster <- seedbank_load_v %>%  # left join seedbank clusters
  left_join(seed.subgrp.df, by = c("stand_core", "stand", "site")) %>% 
  rename(sdbk_clust = sub_grp)

biannual <- seedbank_cluster %>% 
  filter(lifescycle == "biannual") # Acmispon maritimus, Acmispon strigosus
...
# n = 36 = 38 - 2 (for Trifolium spp. and Phacelia spp.) 
...
summary(aov(spp_rich ~ sdbk_clust, forb_spprich_bycore))

#TukeyHSD(summary(aov(spp_rich ~ sdbk_clust, forb_spprich_bycore))

natforb_cluster_spprich <- forb_spprich_bycore %>% 
  group_by(sdbk_clust) %>% 
  summarize(spp_rich_mean = mean(spp_rich), 
            spp_rich_SD = sd(spp_rich)
            )
#  sdbk_clust spp_rich_mean spp_rich_SD
#        <int>         <dbl>       <dbl>
# 1          1          2.88       0.835
# 2          2          5          1.58 
# 3          3          8.22       1.64

## native forb spp in cluster 1
