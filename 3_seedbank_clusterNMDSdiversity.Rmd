...
germination_m2_raw <- read_xlsx("processed/old_processed/germination_m2.xlsx") # does not include pre-treatment
average_cover <- read.csv("nat_nn_cover.csv", header = TRUE, na.strings=c("","NA")) # DEG 1, 2; ENH 1, 2; INT 1, 2, 3

germination_depthtreat <- germination_m2_raw %>% # does not include pre-treatment
  ...
  mutate(species_diversity = case_when(
    ...
    species_diversity %in% c("Erodium cicutarium", "Erodium moschatum") ~ "Erodium spp.",
    species_diversity %in% c("Phacelia viscida", "Phacelia tanacetifolia") ~ "Phacelia spp.",
    TRUE ~ as.character(species_diversity)))
  


glimpse(germination_depthtreat)
sort(unique(germination_depthtreat$species_diversity))
Trifolspp <- germination_depthtreat %>% 
  filter(str_detect(species_diversity, "Trifolium"))
sort(unique(Trifolspp$species_diversity))
...
#### Here is a list of all the species recorded in the seedbank, treatments: control, oven, char, oven_char 
sppchar <- seedbank_load_v %>%  
  ...
  mutate(species = case_when(
    ...
    species %in% c("Erodium cicutarium", "Erodium moschatum") ~ "Erodium spp.",
    species %in% c("Phacelia tanacetifolia", "Phacelia viscida") ~ "Phacelia spp.",
    species %in% c("Trifolium gracelentum", "Trifolium willdenovii") ~ "Trifolium spp.",
    
    # species == "Pseudognaphalium californicum" ~ "Pseudognaphalium luteoalbum" # non-native
    
    
    TRUE ~ species
  ))

sort(unique(sppchar$species))
...
abcover_charateristics_load <-  read.csv("processed/ab_specieslistwithcharateristics_09122025.csv", header = TRUE, na.strings=c("","NA")) %>% 
  dplyr::select(!c(X)) %>% 
...
spp_charateristics <- abcover_charateristics_load %>% 
  ...
  add_row(species = "Typha domingensis",
   ...
   add_row(species = "Malacothrix saxatilis",
           lifecycle = "perennial", 
          lifeform = "forb", 
          status = "native", 
          family = "Asteraceae") %>% 
    add_row(species = "Penstemon heterophyllus",
           lifecycle = "perennial", 
          lifeform = "forb", 
          status = "native", 
...
sort(unique(spp_charateristics$species))



Trifolspp1 <- spp_charateristics %>% 
  filter(str_detect(species, "Trifolium"))
sort(unique(Trifolspp1$species))


##########
#These are the species that are in germination_depthtreat that are NOT in abcover_charateristics
# setdiff(germination_depthtreat$species_diversity, abcover_charateristics_load$species)

#These are the species that are in germination_depthtreat that are NOT in the species list
difgerm <- setdiff(germination_depthtreat$species_diversity, spp_charateristics$species)
difgerm

#These are the species that are in the species list that are NOT in germination_depthtreat
difspplist <- setdiff(spp_charateristics$species, germination_depthtreat$species_diversity)
difspplist # it doesn't matter how many spp are in the list that aren't found in the seedbank
...
germination_depthtreat$site_core_depth_species <- paste(germination_depthtreat$stand_core_depth, germination_depthtreat$species_diversity, sep="_") 
...
########### running CLUSTER with MAX n per g per SPECIES_abundance
seedbank_nperg_bysamplocationspp <- germination_depthtreat %>% 
  group_by(site_core_depth_species) %>% 
  dplyr::summarise(max_n_per_g = max(n_per_g)) %>% # take max germ
  separate(col = site_core_depth_species, into = c("site", "core", "depth", "species"), sep = "_") %>%
   mutate(site_core = paste(site, core, sep = "_")) %>% 
   mutate(site_core_species = paste(site_core, species, sep = "_")) %>% 
  dplyr::select(depth, max_n_per_g, site_core_species) %>% 
  pivot_wider(names_from = depth, values_from = max_n_per_g) %>% 
 mutate(across(c(deep, surface), ~replace_na(.x, 0))) %>% 
  rowwise() %>% 
  mutate(totalgerm_nperg_species = mean(c(surface, deep), na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(check = (deep+surface)/2) %>% 
separate(col = site_core_species, into = c("site", "core", "species_abundance"), sep = "_") %>%
   mutate(stand_core = paste(site, core, sep = "_")) %>% 
  dplyr::select(site, stand_core, species_abundance, totalgerm_nperg_species)
...
### CLUSTER - species
```{r}
# Here you're transposing the data
seedbank_nperg_bysamplocationspp_nmdsprep <- seedbank_nperg_bysamplocationspp %>% 
 tidyr::spread(species_abundance, totalgerm_nperg_species) %>% # transpose data so columns are species names
  replace(is.na(.), 0)

### Here you're replacing NAs with 0s
seedbank_nperg_bysamplocationspp_nmdsprep_num <- seedbank_nperg_bysamplocationspp_nmdsprep %>% 
    ungroup() %>% 
  dplyr::select(!c('site', 'stand_core')) %>% 
  replace(is.na(.), 0)
  
dim(seedbank_nperg_bysamplocationspp_nmdsprep_num) # 26 61
...
# NAME data set
df <- seedbank_nperg_bysamplocationspp_nmdsprep_num
...
# Best method?
map_dbl(m, ac)
# Best method is ward at 0.8069962 (number closest to 1)
...
# Scree plot  
fviz_nbclust(df, FUN = hcut, diss = d, method = "wss", hc_method = "ward.D2")

# Average Silhouette
# Measures how similar an object is to its own cluster (cohesion) compared to other clusters (separation). The Average Silhouette Width is the mean of all silhouette scores within a cluster or across all clusters.
# S ≈ 1: The point is well clustered
fviz_nbclust(df, FUN = hcut, diss = d, method = "silhouette", hc_method = "ward.D2")
# 5 groups has the highest Avg Silhouette

# Gap Stat
# Compares the total within-cluster variation for different numbers of clusters with their expected values under a null reference distribution of the data (i.e., uniformly distributed points).
# The optimal number of clusters is the smallest k such that the Gap Statistic is maximized or the first local maximum occurs, indicating that adding more clusters does not significantly improve the clustering.
fviz_nbclust(df, FUN = hcut, diss = d, method = "gap_stat", hc_method = "ward.D2")
# 2 gruops is best
...
# 5 clusters kinda looks nice
# 3 clusters would also work
...
```{r germination (n/g soil) by family, eval = FALSE}
...
# 2025 by plant cluser

- germination (n/g soil) by plant cluster
```{r}
