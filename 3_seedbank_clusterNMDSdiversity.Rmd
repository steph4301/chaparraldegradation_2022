...
seedbank_nperg_bysamplocation <- germination_depthtreat %>% 
  group_by(stand_core_depth_genus) %>% 
  dplyr::summarise(max_n_per_g = max(n_per_g)) %>% # take max germ
  separate(col = stand_core_depth_genus, into = c("stand", "core", "depth", "genus"), sep = "_") %>%
   mutate(stand_core = paste(stand, core, sep = "_")) %>% 
   mutate(stand_core_genus = paste(stand_core, genus, sep = "_")) %>% 
  group_by(stand_core_genus) %>% 
  dplyr::summarise(totalgerm_nperg_genus = sum(max_n_per_g)) %>% # sum max germ at 0-4 cm with max germ at 4-12 cm
  separate(col = stand_core_genus, into = c("site", "core", "genus_abundance"), sep = "_") %>% 
...
seedbank_nperg_bysamplocation_nmdsprep_num <- seedbank_nperg_bysamplocation_nmdsprep %>% 
    ungroup() %>% 
  dplyr::select(!c('site', 'core', 'stand_core')) %>% 
  replace(is.na(.), 0)
...
#  Group 1  #sps.  1 
#                     A      B  stat p.value  
# Croton setiger 1.0000 0.4444 0.667    0.03 *
# 
#  Group 2  #sps.  5 
#                                 A      B  stat p.value   
# Centaurea melitensis       0.8570 1.0000 0.926   0.005 **
# Malacothamnus fasciculatus 0.7158 1.0000 0.846   0.005 **
# Typha domingensis          0.7786 0.7500 0.764   0.010 **
# Lactuca serriola           1.0000 0.3750 0.612   0.030 * 
# Logfia filaginoides        1.0000 0.3750 0.612   0.020 * 
# 
#  Group 3  #sps.  10 
#                                    A      B  stat p.value   
...
# Claytonia spp.                1.0000 0.6667 0.816   0.010 **
# Malacothrix saxatilis         1.0000 0.5556 0.745   0.010 **
# Ribes spp.                    1.0000 0.5556 0.745   0.010 **
# Phacelia spp.                 0.9270 0.5556 0.718   0.025 * 
# 
#  Group 1+2  #sps.  3 
#                          A      B  stat p.value   
# Hirschfeldia incana 1.0000 0.8824 0.939   0.010 **
# Erodium spp.        1.0000 0.7647 0.874   0.010 **
# Acmispon spp.       0.9504 0.7647 0.853   0.015 * 
# 
#  Group 2+3  #sps.  3 
#                            A      B  stat p.value   
# Artemisia californica 0.9251 0.8824 0.903   0.010 **
# Festuca myuros        0.9788 0.7059 0.831   0.015 * 
# Salvia leucophylla    1.0000 0.5882 0.767   0.025 * 
# ---
