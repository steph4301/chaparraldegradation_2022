...
orders, then a plot of one against the other must be monotonically
...
It can recognize differences in total abundances when relative
abundances are the same


```{r thieftime load data}
...
# this is the data table with column headers and all numbers
view(mite.hel) 
str(mite.hel) # 70 obs by 35 variables
...
1. Load data for NMDS by site_rep
- df: deg_percentcover_bysiterep_nmdsprep_num.csv
```{r load data}
# me!
abcover_bysite_nmdsprep_num_load <- read.csv("~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/chaparraldegradation_2022b/processed/deg_percentcover_bysiterep_nmdsprep_num.csv", header = TRUE, na.strings = c("","NA"))

abcover_bysite_nmdsprep_num_load_clean <- abcover_bysite_nmdsprep_num_load %>% 
  select(!c(X))
head(abcover_bysite_nmdsprep_num_load_clean)
str(abcover_bysite_nmdsprep_num_load_clean)
# 124 obs and 15 variables
# each site_rep should have 124 rows of data (obs)
# there should be 15 variables: 6 DEG + 6 ENH + 3 INT

# stress test, will run 20 times
abcover_siterep_nmds <- metaMDS(abcover_bysite_nmdsprep_num_load_clean, 
                 autotransform = F) 
...
ordiplot(abcover_siterep_nmds) # without point labels
ordiplot(abcover_siterep_nmds, type = "t") # with point labels

#ggvegan to make a quick nice plot
autoplot(nmds1) # NMDS1 x NMDS2, Key = "Score": sites, species

autoplot(abcover_siterep_nmds) # NMDS1 x NMDS2, Key = "Score": sites, species

#this makes a dataframe that gives a lot of control graphically 
fort <- fortify(nmds1)
abcover_siterep_nmds_fort <- fortify(abcover_siterep_nmds)
...
# me!
abcover_siterep_nmds_fortplot <- ggplot() + 
  geom_point(data = subset(abcover_siterep_nmds_fort, Score == 'sites'),
             mapping = aes(x = NMDS1, y = NMDS2),
             color = 'black',
             alpha = 0.5) +
  geom_segment(data = subset(abcover_siterep_nmds_fort, Score == 'species'),
               mapping = aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2),
               arrow = arrow(length = unit(0.015, "npc"),
                             type = 'closed'),
               color = 'darkgrey',
               linewidth = 0.8) +
  geom_text(data = subset(abcover_siterep_nmds_fort, Score == 'species'),
            mapping = aes(label = Label, x = NMDS1*1.1, y = NMDS2*1.1))+
  geom_abline(intercept = 0,slope = 0,linetype = "dashed", linewidth = 0.8,colour = "gray")+
  geom_vline(aes(xintercept = 0), linetype = "dashed", linewidth = 0.8, colour = "gray")+
  theme(panel.grid.major = element_blank(),
...
view(dune)
str(dune)
#20 rows or obs, 30 columns or variables
...
# create a vector of color values corresponding of the same length as the vector of treatment values
# 20 obs and 30 variables == 20 sites and 30 species
