...
# load abcover_charateristics
- the df "abcover_charateristics" is from transect_continuous.Rmd
```{r load data from transect_continuousdotRMD, include = TRUE}
...
START data prep code copy/pasted from transect_continuous.Rmd
```{r data prep load csv, include=FALSE}
...
```{r data prep full join, include=FALSE}
...
```{r data prep species list, include=FALSE}
...
```{r data prep species list with family characteristics, include=FALSE}
...
```{r data prep update species list with family charicteristics}
...
```
END data prep code copy/pasted from transect_continuous.Rmd
...
nmds_above_df_plotcolor <- ggplot(nmds_above_df, 
  ...
  scale_shape_manual(values = c(16, 17, 15, 15, 15,15, 15)) + 
...
#write.csv(fit_above_spp, "~/Dropbox/GRADSCHOOL/Dissertation/R_dissertation/chaparraldegradation_2022/chaparraldegradation_2022/processed/percentcover_nmds_pvalues.csv")
```
...
nmds.1 <- ggplot(plot_above_df, aes(x = NMDS1, y = NMDS2)) +
  coord_fixed() +
    # geom_point(aes(color = site_rep, shape = site), size = 5, alpha = 0.8) + #`Bromus rubens`
  geom_point(size = 3) +
  scale_shape_manual(values = c(15, 18, 8, 8, 8,8, 8)) + 
