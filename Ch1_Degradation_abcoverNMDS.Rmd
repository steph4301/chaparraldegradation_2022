...
# Load csv
### 1. point-intercept transect data for percent cover
...
```
### 2. belt transect data 

```{r load csv spp diversity, include = TRUE}
...
# Renaming observations - another way to rename observations
# ab_diversity <- abcover_full_raw %>%
#   mutate(species = case_when(
#     species == "Avena barbata dead" ~ "Avena barbata",
#     species == "Bromus rubens dead" ~ "Bromus rubens",
...
abcover_full_join <- abcover_full_raw %>% 
 dplyr::select(c(species, lifeform, status, lifecycle, family))
glimpse(abcover_full_join)



ab_diversity <- left_join(ab_diversity_raw, abcover_full_join, by = "species")
#view(ab_diversity)
...
abcover_byspecies_nmdsprep_num <- abcover_byspecies_nmdsprep %>% 
  ...
  ungroup() %>% 
  dplyr::select(!c(standtype, site, site_rep))
...
ab_richness <- abcover_byspecies_nmdsprep_num

ab_richness$"HEAR" <- ab_richness$"Heteromeles arbutifolia" + ab_richness$"Heteromeles arbutifolia dead"

view(ab_richness)
abcover_byspecies_nmdsprep_num_sppr <- abcover_byspecies_nmdsprep_num %>% 
  # rename("new" = "old")
  dplyr::("Heteromeles arbutifolia" = "Heteromeles arbutifolia dead",
         "Bromus rubens"  = "Bromus madritensis",
       #  "Bromus rubens"  =  "Bromus spp. dead", 
      #  "Bromus rubens"  =  "Bromus spp.", 
      #   "Bromus rubens"  = "Bromus rubens dead", 
         "Stipa lepida" = "Stipa lepida dead", 
     #   "Elymus condensatus" = "Elymus condensatus dead",  
        "Salvia leucophylla" = "Salvia leucophylla dead",  
        "Centaurea melitensis"  = "Centaurea melitensis dead",  
        "Hirschfeldia incana" = "Hirschfeldia incana dead" , 
        #"Erodium spp." = "Erodium cicutarium",  
        "Malacothamnus fasciculatus" = "Malacothamnus fasciculatus dead", 
        "Avena barbata" = "Avena barbata dead",
        "Bromus diandrus" = "Bromus diandrus dead")


view(abcover_byspecies_nmdsprep_num_sppr)
...
```
```{r species richness Take 2, include = TRUE}
above_percentcover
...
# nmds - by site_rep
nmdsabove_plot_site_rep <- ggplot(plot_above_df, aes(x = NMDS1, y = NMDS2)) +
  #coord_fixed() +
  geom_point(aes(color = standtype), size = 3, alpha = 0.8) +
