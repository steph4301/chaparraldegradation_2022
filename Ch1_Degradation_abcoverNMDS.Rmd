...
```
### 2. transect data for species richness 
...
```{r species richness by belt transect, include = TRUE}
beltrans_sppr <- read.csv("processed/shrubdensitylife_transposed.csv", header = TRUE, na.strings=c("","NA"))
beltrans_sppr <- beltrans_sppr %>% 
  dplyr::select(!c("X"))

# combine site and rep to one column
beltrans_sppr$site_rep <- paste(beltrans_sppr$site, beltrans_sppr$rep, sep="_") 

glimpse(beltrans_sppr)

write.csv(beltrans_sppr, "processed/beltrans_sppr.csv")
```
...
ab_div_bysiterep <- ab_diversity_2_native_nn %>%
...
### NEED TO FIX CODE - KIT

ab_sppr_nngrass <- ggplot(data = ab_diversity_2_native_nn, 
       aes(x = nonnative_grass*100, y = length(unique(species)), shape = standtype, color = standtype)) + #shape = standtype.x, color = standtype.x
