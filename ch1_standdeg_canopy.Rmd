...
```
```{r aov veg/ground cover by SITE, include=TRUE}
...
##### (B) Percent cover by lifeform by native/nonnative status - there shouldn't be zeros in your df
...
ggplot(data = cover_statuslf_matrix %>% filter((lifeform == 'shrub')) %>% filter((status == 'native')), 
       aes(y = percentcover, x = site_rep, fill = lifeform)) +
    geom_boxplot() # boxplots
...
ggplot(data = cover_statuslf_matrix_wide1, 
 ...
 # scale_fill_manual(values = standcolors) +
  labs(x = "Non-native grass cover (%)", y = "Native shrub cover (%)", title = "Native Shrub Cover by Non-native Grass Cover") +
...
```{r forb native percent cover by site, include=TRUE}
