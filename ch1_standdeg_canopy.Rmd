...
### SEGMENTED ###
<<<<<<< Updated upstream
### Breakpoint analysis, segmented regression ### KIT, THANK YOU! 
p1 <- ggplot(abcover_nonnativecover_nngrasscover, aes(x = nonnative_grass, y = native_shrub)) + geom_line()+
=======
p1 <- ggplot(abcover_nonnativecover_nngrasscover2, aes(x = nonnative_grass, y = percentcover_nonnative)) + geom_line()+
  geom_point()
...
# get the fitted data
my.fitted <- fitted(my.seg)
my.model <- data.frame(NonNativeGrassCover = abcover_nonnativecover_nngrasscover2$percentcover_nonnative, TotalNonNativeCover = my.fitted)
...
# add the fitted data to the exisiting plot
p2 <- p2 + geom_line(data = my.model, aes(x = NonNativeGrassCover, y = TotalNonNativeCover), colour = "tomato")

# add vertical lines to indicate the break locations
# second row of the psi-matrix
my.lines <- my.seg$psi[, 2]

p2 <- p2 + geom_vline(xintercept = my.lines, linetype = "dashed")
p2
```
...
p3 <- ggplot(abcover_nonnativecover_nngrasscover2, aes(y = nonnative_grass, x = percentcover_nonnative)) +
  geom_point()
p3 <- p3 + geom_line(data = my.model1, aes(x = NonNativeGrassCover, y = TotalNonNativeCover), colour = "tomato")
p3
...
ggplot(abcover_byfamily_percent, aes(fill=family, y=percent_cover, x=site_rep)) + 
