...
write.csv(cover_statuslf_matrix_long,"processed/coverstatus.csv")
...
# KIT - here is where I tried to make a y = nonnative_orb by x = native_shrub figure, and got stuck... 
...
for (i in 1:bpcount) { # <-- I got stuck here
  break1[i] <- my.seg$psi[[i+bpcount1]]
  if (i == 1) {
    slope1[i] <- coef(my.seg)[[2]] 
    intercept[i] <- coef(my.seg)[[1]]
    break1[i] <- min(abcover_nativecover_nngrasscover2$nonnative_forb, n = 1)
  }
  else {
    slope1[i] <- slope1[i-1] + coef(my.seg)[[i+1]]
    intercept[i] <- (intercept[i-1] + slope1[i-1]*break1[i]) - (slope1[i]*break1[i])
