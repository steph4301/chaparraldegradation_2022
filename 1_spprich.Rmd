...
############# species 
# omit dupicate species from total spp richness count
length(
  (spprich %>% 
  dplyr::select(species) %>% 
  ...
  filter(!species == "Thysanocarpus spp.") %>% 
  filter(!species == "Trifolium spp.") %>% 
    
  distinct())$species) # n = 87 species
...
spprich %>% 
  dplyr::select(species) %>% 
  mutate(species = case_when( 
      species == "Unknown 3 Grindelia?" ~ "Unknown3Grindelia? Unknown3Grindelia?", # Site 2, transect 2
      species == "Unknown pretty seed" ~ "UnknownprettyseedINT4 UnknownprettyseedINT4",
      species == "unknown collected aster (NB2312 page 123)" ~ "unknowncollectedaster unknowncollectedaster",
      species == "unknown seedling (photo)" ~ "unknownseedlingphotoINT5 unknownseedlingphotoINT5",
      species == "unknown7 forb - unknown oval shaped leaf, red colored stem at the base - no photos" ~ "unknown7forb unknownforb", # found with Omar and Wyatt at ENH2 (aka. Site 2)
       TRUE ~ species)) %>% 
  distinct()
