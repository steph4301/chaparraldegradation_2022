...
# test for normality
hist(spprich_native$grass_nonnative)
qqnorm(spprich_native$grass_nonnative)
qqline(spprich_native$grass_nonnative, col = "red", lwd = 2)

# qq plots of herb, forb, and native grass are a bit skews

shapiro.test(spprich_native$grass_nonnative) # <-- not normally distributed
...
model_gam <- gam(N_spprich ~ s(grass_nonnative), data = spprich_native)
summary(model_gam)
plot(model_gam)
...
model_qr <- rq(N_spprich ~ grass_nonnative, tau = 0.9, data = spprich_native)
summary(model_qr)
...
AIC_totnat <- (AIC(model_lin, model_poly, model_gam, model_seg)) # Best models: GAM, segmented
...
######## richness by status and lifeform - data prep
...
# Here you test whether there’s a breakpoint in grass cover where the relationship with shrubs changes slope:
model_seg <- segmented(model_lin, seg.Z = ~grass_nonnative, # <-- uses linear model from above
                       npsi = 1) # one breakpoint
...
# To decide if non-linear/threshold models are “better” than linear:
# Compare models using AIC:
(AIC(model_lin, model_poly, model_gam, model_seg))
# Lower/more negative AIC = better fit.
...
cor.test(df$grass_nonnative, 
         df$N_spprich, method = "spearman")
# t = 0.54032, df = 16, p-value = 0.5964
# cor 
# 0.1338631 
# rho 
# 0.1149919 
