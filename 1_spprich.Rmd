...
##### y = native spprich
```{r}
# segmented model is the best fit regression. 
# linear model is just as good.
summary(
  segmented(
  lm(N_spprich ~ grass_nonnative, data = spprich_native), seg.Z = ~ grass_nonnative,
                       npsi = 1)
  )
# breakpoint = 14.7% nng
# < 14.7% slope = -68.333; p = 0.0901
# > 14.7% slope = -2.98;  no pvalue

# Plot the segmented regression model and increase text sizes
plot(segmented(
  lm(N_spprich ~ grass_nonnative, data = spprich_native)
...
points(spprich_native$grass_nonnative, 
       spprich_native$N_spprich,
...
spprich_native <- spprich_trans_status %>%  
  filter(status == "native")

spprich_nn <- spprich_trans_status %>%  
 filter(status == "nonnative")


##### native 

plot_spprich_native <- ggplot(data = spprich_native, # or status == "nonnative"
       aes(x = grass_nonnative*100, 
         y = N_spprich)) + 
      geom_point( 
        aes(color = cluster),
...
cor.test(spprich_native$grass_nonnative, 
         spprich_native$N_spprich, method = "spearman")
# NATIVE
# p-value = 0.001723
# cor = -0.4799879 
...
model_poly <- lm(N_spprich ~ poly(grass_nonnative, 2), data = spprich_native)
summary(model_poly)
...
model_lin <- lm(N_spprich ~ grass_nonnative, data = spprich_native) #linear regression (same as above)
...
model_lm2 <- lm(N_spprich ~ grass_nonnative, data = spprich_native %>%  filter(N_spprich < 16)) 
model_seg2 <- segmented(model_lm2, seg.Z = ~ grass_nonnative,
                       npsi = 1) # one breakpoint
...
AIC_totnat <- (AIC(model_lin,model_poly, model_seg
                   # , model_lm2,  model_seg2
                   )) # Best models: GAM, segmented
# Lower/more negative AIC = better fit.
AIC_totnat
summary(model_seg) # 0.147
plot(model_seg)
...
spprich_trans_status_lf


# native herb spprich
herb_spprich <- spprich %>% 
   # here I am creating the "herb" category (native and nonnative)
  filter(lifeform =="forb" | lifeform == "grass" | lifeform == "forb or vine") %>% 
  group_by(site_rep, status) %>% 
  dplyr::summarize(N_spprich = length(species)) %>% 
  mutate(lifeform = "herb") %>% 
  # here I am joining the "herb" data with the other lifeform data 
  bind_rows(spprich %>%  
  distinct() %>% 
  group_by(site_rep, status, lifeform) %>% 
  summarize(N_spprich = length(species))
  ) %>%
  # here I am joining the % cover data by site_rep
  left_join(
    (herb_cover_wide %>% 
       mutate(site_rep = case_when(
         site_rep == "INT5_5" ~ "INT5_1", TRUE ~ site_rep))
...
# the best regression is: segmented
# A linear regression is equally as good
summary(segmented(
  lm(N_spprich ~ grass_nonnative, data = herb_spprich %>% filter(lifeform == "herb"))
    , seg.Z = ~ grass_nonnative,
                       npsi = 1))
# intercept:5.1686
# breakpoint: 61%
# slope: 1.9603  ; p = 0.394
# slope: -2.4319

# Plot the segmented regression model and increase text sizes
plot(segmented(
  lm(N_spprich ~ grass_nonnative*100, data = herb_spprich %>% filter(lifeform == "herb"))
    , seg.Z = ~ grass_nonnative,
                       npsi = 1), 
     conf.level = 0.95, 
     shade = TRUE,
     # main = "Segmented Regression with Confidence Interval",
     xlab = "Non-Native Grass Cover (%)", 
     ylab = "Native Herbaceous Richniess (#)",
     cex.main = 1.5,   # Increase the main title size
     cex.lab = 1.3,    # Increase the axis labels size
     cex.axis = 1.2)   # Increase the axis numbers size
# Add the original data points, increasing their size as well
points(herb_spprich$grass_nonnative, 
       herb_spprich$N_spprich,
       col = colors_plantclusters,
       pch = 16,
       cex = 1.5)      # Increase the size of the data points
...
plot_herb_spprich <- ggplot(data = herb_spprich %>% 
                               filter(status == "native" & 
                                        lifeform == "herb"),
  ...
  labs(
    x = "Non-native grass cover (%)",
    y = "Native herbaceous species richness (n)", 
   color = "Plant cluster"
    ) +
  scale_x_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 10)) + # Set x-axis limits and breaks
  scale_y_continuous(limits = c(0, 30), breaks = seq(0, 30, by = 5), expand = expansion(mult = c(0, 0.05))) + # Set y-axis limits and breaks
...
# Takeaway:
# Correlations - this is a linear correlations
# Linear regression
...
df <- herb_spprich %>% 
  filter(status == "native" &  lifeform == "herb")

hist(df$N_spprich)
shapiro.test(df$N_spprich) # <- normally distributed
# Correlation - Compute a correlation coefficient to see if the two are associated
cor.test(df$grass_nonnative, 
         df$N_spprich, method = "pearson")
# t = -0.75195 <-- likey by chance
# df = 37
# p-value = 0.4568 <-- not sig
# cor 
# -0.1226853
...
AIC_herb <- (AIC(model_lin, model_poly, model_seg))
# Lower/more negative AIC = better fit.
# - segmented is best but a linear regression is just as good of a fit too.


```

##### y = native forb spprich
```{r}

# the best regression is a : segmented regression. 
# Linear regression was just as good.

plot_forb_spprich <- ggplot(data = herb_spprich %>% 
                               filter(status == "native" & 
                                        lifeform == "forb"),
  ...
  labs(
    x = "Non-native grass cover (%)",
    y = "Native forb species richness (n)", 
   color = "Plant cluster"
    ) +
  scale_x_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 10)) + # Set x-axis limits and breaks
  scale_y_continuous(limits = c(0, 30), breaks = seq(0, 30, by = 5), expand = expansion(mult = c(0, 0.05))) + # Set y-axis limits and breaks
...
df <- herb_spprich %>% 
  filter(status == "native" &  lifeform == "forb") 

hist(df$N_spprich)
shapiro.test(df$N_spprich) # <- not normal

# Correlation - Compute a correlation coefficient to see if the two are associated
cor.test(df$grass_nonnative, 
         df$N_spprich, method = "spearman")
# p-value = 0.731
# rho = 0.05686068

# linear regression
model_lin <- lm(N_spprich ~ grass_nonnative, data = df) 
summary(model_lin)
# Intercept <-- when non-native grass cover = 0, the native forb spprich is predicted to be about 5.2119


# Polynomial Regression (Curvilinear Trends) - Fit a model where shrub cover changes in a curve (concave or convex) with grass cover.
model_poly <- lm(N_spprich ~ poly(grass_nonnative, 2), data = df)
summary(model_poly)
...
# To decide if non-linear/threshold models are “better” than linear:
# Compare models using AIC:
AIC_forbspprich <- (AIC(model_lin, model_poly, model_seg))
# Lower/more negative AIC = better fit.
...
##### y = native shrub spprich
```{r}

### segmentted model is the best fit regression
summary(
  segmented(
  lm(N_spprich ~ grass_nonnative, 
     data = spprich_trans_status_lf %>% 
  filter(status == "native" & lifeform == "shrub")), seg.Z = ~ grass_nonnative,
                       npsi = 1)
  )
# breakpoint = 19.5% nng
# intercept =  8.0913
# slope = -19.9693; p = 0.116 
# slope = -2.4636;  no pvalue
...
plot(segmented(
  lm(shrub_native ~ grass_nonnative, data = cover_bylfstatus_wide)
    , seg.Z = ~ grass_nonnative,
     ...
     # main = "Segmented Regression with Confidence Interval",
     xlab = "Non-Native Grass Cover (%)", 
     ylab = "Native Shrub Species Richness (#)",
     cex.main = 1.5,   # Increase the main title size
     cex.lab = 1.3,    # Increase the axis labels size
     cex.axis = 1.2)   # Increase the axis numbers size
...
df <-  spprich_trans_status_lf %>% 
  filter(status == "native" & lifeform == "shrub")

# Correlation - Compute a correlation coefficient to see if the two are associated
cor.test(df$grass_nonnative, 
         df$N_spprich)
# t = -6.0769, df = 35, p-value = 6.116e-07
#        cor 
# -0.7165224 


# linear regression
lm(N_spprich ~ grass_nonnative, data = df) 
...
# Generalized Additive Models (GAMs) - If you don’t want to assume the shape (e.g., parabola), GAMs let the data “decide” the curve:
model_gam <- gam(N_spprich ~ s(grass_nonnative), data = df)
summary(model_gam)
plot(model_gam)
# - GAMs are flexible and can capture non-linear but smooth patterns.
# - You can test significance of the smooth term (edf > 1 indicates non-linearity).
...
# Threshold or Breakpoint Models (Piecewise Regression)
# Here you test whether there’s a breakpoint in grass cover where the relationship with shrubs changes slope:
model_lin <- lm(N_spprich ~ grass_nonnative, data = df) #linear regression (same as above)
model_seg <- segmented(model_lin, seg.Z = ~grass_nonnative,
                       npsi = 1) # one breakpoint
summary(model_seg)
plot(model_seg)
...
# Quantile Regression - Sometimes the average trend isn’t the full story — for example, shrubs may persist under moderate grass cover only in the “best-case” sites. Quantile regression shows how the relationship differs across the distribution of shrub cover.
model_qr <- rq(N_spprich ~ grass_nonnative, tau = 0.9, data = df)
summary(model_qr)
# - This tests whether the upper limit of shrub cover declines with grass cover.
...
# To decide if non-linear/threshold models are “better” than linear:
# Compare models using AIC:
AIC_shrubspprich <- (AIC(model_lin, model_poly, model_gam, model_seg))
# Lower/more negative AIC = better fit.

AIC_shrubspprich
```

##### y = native grass spprich
```{r}

# linear model is the best fit regression
summary(lm(N_spprich ~ grass_nonnative, data = spprich_trans_status_lf %>% 
                               # filter(grass_nonnative < .6) %>% 
                               filter(status == "native" & lifeform == "grass")) )

### grass
plot_spprich_grass <- ggplot(data = spprich_trans_status_lf %>% 
                               # filter(grass_nonnative < .6) %>% 
                               filter(status == "native" & lifeform == "grass"),
  ...
  labs(
    #  title = "Linear regression of  total shrub cover by non-native grass cover",
    x = "Non-native grass cover (%)",
    y = "Native grass speices richness (n)", 
   color = "Plant cluster"
    ) +
  scale_x_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 10)) + # Set x-axis limits and breaks
  scale_y_continuous(limits = c(0, 30), breaks = seq(0, 30, by = 5), expand = expansion(mult = c(0, 0.05))) + # Set y-axis limits and breaks
...
hist(df$N_spprich)
shapiro_test(df$N_spprich) # <-- not normal
...
# linear regression
lm(N_spprich ~ grass_nonnative, data = df) 
# Intercept <-- when non-native grass cover = 0, the native herb spprich is predicted to be about 7.023
# grass_nonnative <-- slope, for every 1% increase in grass cover, native herb spprich decreases by about 1 species.
...
# To decide if non-linear/threshold models are “better” than linear:
# Compare models using AIC:
AIC_grassspprich <- (AIC(model_lin, model_poly, model_gam, model_seg))
# Lower/more negative AIC = better fit.
# - GAM is best fit, but polynomial and segmented regressions are just as good of a fit too.
...
##### FIGURE - native spprich
```{r}


fig_spprich_total <- ggdraw(plot_spprich_native) + draw_label("A", x = 0.95, y = 0.92, size = 15)
fig_spprich_shrub <- ggdraw(plot_spprich_shrub) + draw_label("B", x = 0.95, y = 0.92, size = 15)
fig_spprich_herb <- ggdraw(plot_herb_spprich) + draw_label("C", x = 0.95, y = 0.92, size = 15)  
fig_spprich_forb <- ggdraw(plot_forb_spprich) + draw_label("D", x = 0.95, y = 0.92, size = 15) 
fig_spprich_grass <- ggdraw(plot_spprich_grass) + draw_label("E", x = 0.95, y = 0.92, size = 15) 
 

# Combine the plots in a 2x3 layout
figure_spprich_native <- plot_grid( fig_spprich_total, fig_spprich_shrub,
             fig_spprich_herb, fig_spprich_forb,
             fig_spprich_grass, NULL,
             nrow = 3, 
             ncol = 2, 
...
######## richness by status and lifeform and lifecycle
